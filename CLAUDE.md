...
**Phase 번호는 세 ROADMAP에 걸쳐 이어진다** — 1차 0~6(`docs/home_reader_setup/ROADMAP.md`), 2차 7~21·26·29~30(`docs/payment_relay/ROADMAP.md`), 3차 22~25·27~28(`docs/operations/ROADMAP.md`). 같은 앱을 계속 확장하는 것이므로 번호를 새로 시작하지 않는다.
...
- `.claude/agents/pos-onecap-spec-expert.md` — POS ↔ KFTCOneCAP 간 전문(telegram) SPEC 확인 전담. 근거 문서는
  `docs/payment_relay/spec/국세 베리어프리 키오스크용 전산설계서(POS-원캡)_20260922 (1).pdf`(hwp 원본은 DRM
  배포용 문서 래퍼라 열 수 없음, PDF가 유일하게 파싱 가능 — **2026-09-22판의 재배포본이 최신 정본**이며,
  그 이전 20260922(재배포 전)·20260921·20260915(재배포본 포함)·20260831·20260826판은 저장소에 더는 없다.
  개정 내용은 `docs/payment_relay/PRD.md` §3.3.1 참고 — 3전문 필드 구성·POSITION·길이는 불변이고, 902614의
  `#38`(카드소유주 주민/사업자등록번호) SET 장소가 kiosk→인터넷지로로, `#51`(암호화된 비밀번호 정보)이
  표 원문 자체가 원캡으로 바뀌었다(둘 다 2026-09-21 반영). 800000의 `#11`(이용기관 발행기관 분류코드)/
  `#12`(이용기관 지로번호)는 2026-09-22 개정으로 SET 장소 표시가 전부 삭제돼 kiosk가 채우지 않는
  필드로 확정됐다. 그 이전 20260915판에서는 999999 전문 정식 등재 / 원캡 응답코드(E·R·D) 섹션 신설 /
  서버 응답코드 `031` 추가가 있었다.
  **전문 간 필드 연쇄는 필드 출처(어느 응답 필드에서 오는지)는 사용자가 전부 확정했으나 SPEC
  원문 명문 근거는 대부분 없다**(SPEC은 이 저장소 개발팀이 직접 작성·관리하는 문서다 — 외부 발주처
  질의 대장이 아니다, 2026-09-22 정정) — 조사 기록과 확정 상태는
  ...
  눈으로 훑으면 SET 장소 열을 착각하기 쉽다(최초 정리 때 #48을 kiosk로 잘못 읽은 전례, 이후 #51을
  kiosk로 잘못 읽어 902614를 7개로 캐시했던 전례가 있다 — 2026-09-01 재확인으로 8개 확정).

...
**완료 조건**
- [ ] `dotnet build` 후 생성된 실행 파일의 매니페스트에 `requireAdministrator`가 박혀 있다
      (`manifest.exe` 또는 속성 확인)
- [ ] 관리자 권한 없이 실행하면 UAC 프롬프트가 뜬다(수동 확인)
- [ ] 앱 기동 후 `C:\KFTC_PosAgent\KFTCTaxLog\yyyy-MM-dd.log`가 생성된다
- [ ] 기존 `%LOCALAPPDATA%\KFTCTaxGiroCAP\logs\`에는 **새 파일이 생기지 않는다**(경로가 완전히
      전환됐는지 확인)
- [ ] `%LOCALAPPDATA%\KFTCTaxGiroCAP\`의 SQLite DB는 그대로 그 자리에서 정상 동작한다
...
**완료 조건**
- [ ] `LogRecord` + `LogCategory` + 렌더러가 `Services/Diagnostics/`에 존재하고, WPF 타입에
      의존하지 않는다(공통 규칙 4)
- [ ] 값이 전부 채워진 레코드와 전부 비어 있는 레코드가 각각 `PRD.md` §1.3-b의 예시와 같은 문자열로
      렌더링된다
- [ ] **여러 줄 메시지(스택 트레이스)를 넣어도 출력이 정확히 1줄**이다
- [ ] `PRD.md` §1.3-b의 파싱 정규식으로 위 출력들을 되파싱했을 때 원래 필드 값이 그대로 복원된다
...
**완료 조건**
- [ ] 16자리 카드번호를 포함한 메시지가 `123456******7890` 형태로만 파일에 남는다
- [ ] 13자리/19자리 경계값도 마스킹된다. **12자리 이하는 마스킹하지 않는다**(금액·전화번호·
      전문관리번호가 뭉개지면 안 된다) — 경계 동작을 실제로 확인한다
- [ ] 한 메시지 안에 숫자열이 여러 개 있어도 전부 마스킹된다
- [ ] 마스킹이 파이프라인의 **단일 지점**에 있음을 코드로 확인(싱크마다 따로 하지 않는다)
...
**완료 조건**
- [ ] `FileLogger.Info/Warn/Error(string)` 시그니처가 그대로이고, **호출부 151곳이 무수정**이다
      (`git diff`로 확인 — 호출부 파일에 변경이 없어야 한다)
- [ ] 앱을 기동해 로그가 기존과 같은 경로·파일명으로 남는다
- [ ] 싱크가 예외를 던지도록 강제로 만들어도 앱이 죽지 않고 호출자에게 전파되지 않는다
- [ ] 여러 스레드에서 동시에 대량 기록했을 때 줄 섞임·깨짐이 없다(진단 하네스로 확인)
- [ ] 로그 파일을 다른 프로세스가 열어둔 채로도 기록이 계속된다
...
**완료 조건**
- [ ] 501건을 기록하면 가장 오래된 1건이 밀려나고 크기가 500으로 유지된다
- [ ] 거래ID로 필터링해 해당 거래의 레코드만 시간순으로 꺼낼 수 있다
- [ ] 다중 스레드 기록 중 스냅샷을 떠도 예외가 나지 않는다
- [ ] 링버퍼의 내용이 **이미 마스킹된 상태**다(P22-2가 앞단이므로 자동이지만 실제로 확인한다)
...
**완료 조건**
- [ ] 91일 전 날짜 파일명으로 더미 파일을 만들어 두면 기동 시 삭제된다
- [ ] 89일 전 파일은 **남는다**(경계 확인)
- [ ] `readme.txt`처럼 패턴에 맞지 않는 파일은 남는다
- [ ] 삭제 대상 파일을 다른 프로세스가 열어 잠가 두면, 예외 없이 건너뛰고 앱이 정상 동작한다
- [ ] 자정을 넘겨 로그가 새 파일에 기록될 때 정리가 다시 한 번 돈다(시스템 시각을 조정하거나
      날짜 전환 훅을 직접 호출해 확인 — 어느 방법으로 확인했는지 적는다)
...
**완료 조건**
- [ ] 결제 1건 실행 시 위 표의 모든 경계가 로그에 나타나고, 그 줄들의 거래ID가 **전부 같다**
- [ ] 실패 거래에서 POS에 나간 결과 코드와 로그의 코드 슬롯 값이 **일치**한다
- [ ] 메시지 문자열 안에 `txId=`가 중복으로 남아 있지 않다(전용 필드로 옮겼으므로)
- [ ] 손대지 않은 나머지 호출부는 `[-] [-] [-]`로 정상 기록된다
...
**완료 조건**
- [ ] 거래 1건 후 해당 포트의 `reader_auth_id`가 저장돼 있고 `observed_at`이 그 시각이다
- [ ] 상태체크 버튼으로도 같은 값이 갱신된다
- [ ] 같은 포트로 두 번 관측하면 행이 늘지 않고 **덮어써진다**(upsert 확인)
- [ ] 리더기 2대 구성에서 포트별로 각각 저장된다
- [ ] DB 파일을 읽기 전용으로 만들어 저장을 실패시켜도 **결제가 정상 완료**된다

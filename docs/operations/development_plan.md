...
| 3 | 슬라이스 기준 | **거래ID와 시간창 둘 다 제공**(§1.8.3-a). 하나로는 부족하다 |
...
| 7 | 시간창 기본 크기 | **함수 인자로 받는다** — 리더는 값을 정하지 않는다. 실제 보고 범위는 "직전 거래 3건부터 로그 끝까지"로 확정됐고(§1.8.3-a2) 그 값은 `fault_alert_catalog.md` §3이 관리한다 |
...
| 12 | 슬라이스 기준 조합 | **직전 거래 3건의 시작점부터 로그 끝까지**(2026-09-10 사용자 확정, §1.8.3-a2 — 시간창 안에서 변경). **리더 계약은 안 바뀐다** — 리더는 거래ID/시간창 두 기준을 각각 제공하고, "직전 N건" 탐색은 호출부(Phase 28)가 한다. **P27-2/P27-3 요구사항은 이 결정의 영향을 받지 않는다** |
...
- 네 슬롯(레벨/카테고리/코드/거래ID)은 고정폭 패딩이 있으므로 **비교 전 `Trim()`**.
...
- [ ] 고정폭 패딩이 붙은 슬롯이 `Trim()` 후 원래 값과 일치한다(레벨 `INFO `, 카테고리 `APP     ` 등).
...
- [ ] 파일명 정규식 공유 방식 결정을 이 Task 아래에 기록했다.

---

## P27-3. `LogFileReader` — 슬라이스 두 종

P27-2의 읽기 위에 조건 필터를 얹는다. **두 기준을 모두 제공한다** — 어느 하나로는 부족하다.

| 기준 | 무엇을 잡나 | 왜 필요한가 |
|---|---|---|
| **거래ID** | 5번 슬롯 일치 | "이 결제가 왜 실패했나" |
| **시간창** | 1번 슬롯이 구간 내 | 거래ID가 `-`인 줄(앱 기동, DLL 로드, 로그 정리 등)까지 포함. **이게 없으면 앱이 기동조차 못 하는 장애가 보고에 안 실린다** |

- **반환은 텍스트 원문**이다. 파싱은 필터링에만 쓰고 원문 줄을 그대로 이어 붙인다(§1.8.3-c).
  구조화 객체로 바꿔 돌려주면 서버가 "실제로 뭐라고 찍혔나"를 복원할 수 없다.
- **빈 줄은 결과에 포함한다**(확정 사항 #5). `FileLogSink`가 거래 확정(`PAYMENT` +
  `[PaymentOrchestrator] 거래 확정`으로 시작) 뒤와 리더기 설정 액션 종료(`UI` + `처리 종료`로 끝남)
  뒤에 넣는 구분선이다. 파서는 매치하지 못하지만 결과 텍스트에는 남겨 사람이 거래 경계를 보게 한다.
  - 다만 **필터에 걸린 줄 사이에 있는 빈 줄만** 포함한다 — 무관한 구간의 빈 줄까지 끌어오지 않는다.
- **시간창 크기는 인자로 받는다.** 기본값을 코드에 박지 않는다(확정 사항 #7).
- 결과가 없으면 빈 문자열. `null`을 돌려주지 않는다.

**완료 조건**
- [ ] 거래ID 슬라이스가 그 거래의 줄만, 시간순으로, 원문 그대로 뽑는다.
- [ ] 시간창 슬라이스가 거래ID `-`인 줄(예: `애플리케이션 기동 시작`)까지 포함한다.
- [ ] 거래 확정 뒤의 빈 줄이 결과에 남는다.
- [ ] 필터 구간 밖의 빈 줄은 결과에 들어오지 않는다.
- [ ] 거래ID fallback 값(12자 초과)으로도 정확히 슬라이스된다.
...
| # | 항목 |
|---|---|
| 1 | 거래ID 슬라이스가 그 거래의 줄만 시간순으로 뽑는다 |
| 2 | 시간창 슬라이스가 거래ID `-`인 줄까지 포함한다 |
...
**완료 조건**
- [ ] 두 파일이 저장소에 없고, `LogRingBuffer`/`RingBufferSink` 참조가 코드에 0건이다.
- [ ] `dotnet build` 경고 0 / 오류 0.
- [ ] 앱이 정상 기동하고 로그가 파일에 그대로 남는다.
- [ ] `ILogSink` 인터페이스와 `ConfigureSinks` 시그니처가 유지됐다.
- [ ] 장시간 실행 시 메모리 증가가 없다(링버퍼가 없어졌으므로 당연하지만 확인한다).
...
**완료 조건**
- [ ] `LogMessageMasker.cs`가 없고 참조가 0건이다.
- [ ] `dotnet build` 경고 0 / 오류 0.
- [ ] **전문 원문 로그에서 전문구분이 온전히 보인다** — `IGN0950200902614`처럼 `902614`가 가려지지
      않는다(Phase 22~26에서는 `IGN095020***2614`였다).
- [ ] **응답 로그에서 응답 코드와 전송 일시가 온전히 보인다**(이전 `C000260******540`).
- [ ] `#46`이 채워진 902614에서 **위치 기반 마스킹은 여전히 동작한다**(앞 6바이트 + `*`).
- [ ] `TelegramLogRedactor` 클래스 주석의 두 부정확한 서술이 고쳐졌다.
...
**완료 조건**
- [ ] 카드/PIN 전수 점검 **위반 0건**, 결과 표를 이 Task 아래에 기록.
- [ ] 전문구분·응답 코드가 온전히 보이는 실제 로그 줄을 인용해 기록.
- [ ] `#46` 위치 기반 마스킹이 동작하는 실제 로그 줄을 인용해 기록.
- [ ] 실장비 결제 1건 성공 로그(시각 포함).
- [ ] 기존 하네스 5종 전량 통과(각 통과/실패 건수 기록).
- [ ] 장시간 실행 시 핸들·메모리 증가 없음.
...
1. 거래ID·시간창 두 기준으로 로그 조각을 파일에서 뽑을 수 있고, 하네스 8항목 전부 통과.

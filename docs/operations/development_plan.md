...
**완료 조건**
- [x] 영향 범위 표가 실제 변경 파일과 일치한다 — `PRD.md` §1.8.5/§1.9.7 2026-09-15 최종 갱신
      (P27-8/9가 추가로 건드린 `LogLevel.cs`/`ILogSink.cs`/`FileLogSink.cs`/`FileLogger.cs`/
      신규 `FaultAlertJudge.cs`/`InternalFaultAlertConditions.cs`/`InternalFaultCodes.cs` 반영).
- [x] `PRD.md` §5 미확정 #10이 해소 또는 상태 갱신됐다 — 해소(취소선), 복사 재사용 결정 기록.
- [x] `fault_alert_catalog.md` §2 표와 실제 판정 결과가 일치함을 P27-9에서 확인했고, 어긋난
      부분이 있었다면 **카탈로그를 정본으로 삼아** 코드를 고쳤다(반대가 아니다) — CP3에서
      `FaultAlertJudge.Classify` 전수 대조 재확인, 어긋남 없음.
- [x] `fault_alert_catalog.md`의 `잠정(제안)` 항목(상한 가드)이 확정됐거나, 확정 대기 사유가
      변경 이력에 남아 있다 — 상한 가드는 이미 `확정`(§3, 200줄). 나머지(`E05`/`R0x`/`S09`/
      `E40`~`E43` 임계값, 슬라이스 범위)는 의도적으로 `잠정`(운영 로그 4주 축적 후 재검토) 유지 —
      §6 규칙대로 값이 채워져 있으므로 완료 조건 충족.
- [x] `PRD.md` §5 미확정 #13/#14/#15가 **해소(취소선) 상태로 유지**된다 — 확인 완료, 되돌리지
      않음. #16(`E42`/`E43` POS 통보) + #17(R코드 재배치 POS 통보) 상태를 "구현·실측 완료, 담당자
      통보만 남음"으로 갱신.
- [x] **`fault_alert_catalog.md` §7 변경 이력에 이번 구현 결과가 반영**됐다 — R코드 재배치,
      `LogFileReader` 계약 축소 등 구현 중 결정이 전부 실시간으로 기록됨.
- [x] 카탈로그 §2.2의 `S` 코드 발생 지점 줄 번호가 **구현 후 실제 위치와 일치**한다 — 2026-09-15
      grep으로 재확인 후 갱신 완료.
- [x] `ROADMAP.md` Phase 27 체크박스가 전부 채워졌다 — P27-1~9 전부 `[x]`, 완료 기준 12개
      전부 실측 근거와 함께 `[x]`, CP1/CP2/CP3 통과 기록 완료.
- [x] `CLAUDE.md`가 §4·Phase 27까지 반영한다 — 2026-09-15 갱신 완료.
- [x] 대체된 절의 원문이 그대로 남아 있다(삭제되지 않았다) — §1.3-b/d/e·§1.4·§1.6.1 등 원문 보존
      확인, 삭제 0건.
...
1. [x] "직전 N건부터 EOF까지" 슬라이스로 로그 조각을 파일에서 뽑을 수 있고, 하네스 8항목 전부 통과 — P27-4.
2. [x] 리더 때문에 로그 기록이 실패하지 않는다(실측) — P27-2/P27-4.
3. [x] 전문구분·응답 코드가 로그에서 가려지지 않는다(실측 인용) — P27-6/P27-7.
4. [x] 카드/PIN 전수 점검 위반 0건 — P27-7(실카드 결제 포함).
5. [x] 실장비 결제 1건 회귀 없음 — P27-7(2026-09-14 10:45:23).
6. [x] `LogRingBuffer` / `RingBufferSink` / `LogMessageMasker` 세 파일 부재, `dotnet build` 경고 0/오류 0 — P27-5/P27-6, 2026-09-15 최종 재확인.
7. [x] **운영 코드에 카테고리 없는 `FileLogger` 호출 0곳**(P27-8-a) — `Info`까지 포함해 최종 40곳
      확인(CP3 리뷰로 Info 13곳 누락 발견·2026-09-15 보완 완료).
8. [x] **`E99`가 로그 code 슬롯에 남고**, `D02`/`R24`~`R32`/`E99` 거래 실패가 `ERROR` 레벨이다(P27-8-b·c).
9. [x] **`S01`~`S11`이 카탈로그 §2.2의 지점에 부여됐고 `S12` 이상은 없다**(P27-8-e) — 2026-09-15
      최종 줄 번호 재확인 완료.
10. [x] **`E42`/`E43`이 실제로 POS에 회신되고 정상 전문 회귀가 없다**(P27-8-f) — CP3에서 원시 소켓
       재현으로 재확인.
11. [x] **알림 판정이 카탈로그 §2 표와 일치하고, `ALERT` 레벨 줄이 남는다**(P27-9) — off-by-one
       버그 수정 후 코드별 실측 완료.
12. [x] **판정이 결제 경로를 막지 않는다** — 인위적 지연을 넣어도 결제 왕복 시간이 늘지 않는다
       (P27-9-e) — CP3에서 독립 재현으로 재확인.
...
**[x] 2026-09-15 — CP1/CP2/CP3 전부 통과(치명적 0건).** CP1에서 청크 경계 CRLF·자정 경계 가드
버그 2건, CP3 이전 설계 단계에서 R코드 값 충돌·판정 이중 카운팅 위험을 자체 발견해 전부 수정.

**[x] 2026-09-15 — Phase 27 전체 최종 종합 검증(P27-1~9 전체 재검증, 완료 선언 직전 마지막 관문)
완료, 치명적 결함 1건 발견·수정.** CP1/2/3 세 번의 Opus 리뷰가 전부 놓쳤던 결함이다 —
`TelegramLogRedactor`가 필드 **번호**만 보고 마스킹해 902614 전용이어야 할 마스킹이 501008/800000
전문까지 적용되고 있었다(501008의 `#14` 전자납부번호·`#36` 교통세가 카드정보로 오인돼 가려짐).
902614 전용 게이트 한 줄로 수정, 902614 마스킹은 그대로 유지, 501008/800000은 원문 그대로
남도록 사용자 확정(`fault_alert_catalog.md` §7 참고). 그 외 A~F(파일 슬라이스 재검증, 로그
코드 체계 전수 대조, 판정 엔진 코드-카탈로그 1:1 대조, 19개 호출 지점 재확인, 비차단 재실측,
전체 하네스 6종 회귀, 문서-코드 정합성 무작위 표본 검증)는 전부 통과 — 상세는 이 완료 선언
직전 세션의 검증 기록을 따른다. **왜 CP1~3이 놓쳤는가**: 세 리뷰 모두 카드/PIN 유출이라는
902614 관점에만 집중했고, 다른 전문 종류(501008/800000)로 교차 검증하지 않았다 — 앞으로 유사한
"필드 번호 기반" 로직을 검증할 때는 반드시 그 로직이 적용되는 모든 전문 종류를 하나씩 대입해
확인해야 한다는 교훈이다.

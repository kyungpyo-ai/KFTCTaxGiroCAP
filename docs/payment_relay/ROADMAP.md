...
- [x] 항상 최상단 표시(`Topmost`) — 됨. **취소 버튼은 화면에 있으나 클릭 핸들러가 비어 있다**(TODO,
      P13-2 취소 로직 이번 라운드에서 미착수).
...
"Phase 13 시각 구현 — 중간 정리(2026-08-24)" 참고. **P13-2(취소 1회 제한)/P13-4(표시 정책)/P13-5(ESC
전역 훅)/P13-6(제어 진입점 계약)/P13-7(시나리오 검증)은 아직 착수 전**이며, 알림창 정교화 재개와 함께
이어서 진행할 예정이다.

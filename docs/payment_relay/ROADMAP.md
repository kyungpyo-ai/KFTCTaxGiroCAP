...
- [ ] **★ 재검토 필요(2026-08-24, P13-4에서 보류)**: 사용자가 `ReaderSetupWindow`(리더기 설정, 모달)를
      연 채로 결제 요청이 들어오는 경우의 정책 미정 — 알림 자체를 보류/거부할지, 그냥 뒤로 보내기만
      할지 결정 필요(리더기 포트를 설정 화면과 결제 워커가 동시에 쓰려는 충돌과도 연결됨). 상세는
      `development_plan.md` P13-4 "알려진 범위 밖" 참고.

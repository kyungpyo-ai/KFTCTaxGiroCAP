...
- [ ] `501008` 응답에 `#17`/`#19`/`#30`/`#31`/`#32`/`#44`/`#55` 등 디지털예산 담당 필드가 공백이 아닌
      값으로 채워져 돌아온다(로그 원문으로 확인).
- [ ] `800000 #14`(BIN)가 **원캡이 카드리딩으로 채운 값 그대로** 응답에 남아 있다(스텁이 덮지 않았다).
- [ ] `902614`의 원캡 담당 8개 필드도 스텁이 덮지 않는다.
- [ ] 응답 본문 길이가 스키마 총 길이(706/500/1500)와 정확히 일치한다(`0x00` 없음).
- [ ] 기존 회귀(`--pos-client-test`, `--payment-flow-test`)가 그대로 통과한다 — 스텁 응답이 바뀌면
      기존 하네스의 기대값이 깨질 수 있으므로 **이 확인이 특히 중요하다.**

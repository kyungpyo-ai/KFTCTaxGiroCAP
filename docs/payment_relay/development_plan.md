# 실행계획서: 결제 중계 기능 (Phase 7~21, 26, 29, 30)

> `PRD.md`(무엇을) → `ROADMAP.md`(어떤 순서로) → **이 문서(Task 단위로 무엇을 어떻게, 어디까지 하면 끝인지)**.
> 실제 코드 작성은 이 문서의 Task를 순서대로 따라간다.
>
> **Phase 12부터는 한 Phase씩 착수 직전에 작성**한다(2026-08-20 사용자 확정) — 앞 Phase의 실장비 검증
> 결과에 따라 뒤쪽 계획이 조정될 여지가 커서, Phase 7~11처럼 여러 Phase를 미리 써두면 앞 결과에 따라
> 다시 고쳐야 할 계획이 생긴다. 현재 수록된 Phase: **7~21, 26, 29, 30**(번호는 세 ROADMAP에 걸쳐 이어지며
> 22~25·27~28은 3차 범위 `docs/operations/`에 있다).
...
# Phase 26 실행계획서 — 거래 상태 조회 전문 + 응답 카드필드 삭제

> 근거: `PRD.md` §3.4(거래 상태 조회 전문), §4.13(응답 전문의 카드리딩 필드 삭제), §7.1(직전 거래 응답
> 저장), §4.11(`E07`) / `ROADMAP.md` Phase 26.
>
> **Phase 22~25는 3차 범위**(`docs/operations/`)라 이 문서에 없다 — 번호는 세 ROADMAP에 걸쳐 이어지므로
> Phase 21 다음이 26이다.

**이 Phase가 끝나면**: 소켓이 끊겨 POS가 응답을 못 받았을 때, POS가 **직전 거래의 결과를 다시 물어봐
그대로 받아갈 수 있다.** `902614`를 재전송하지 않고 복구하므로 이중 승인 위험이 사라진다. 함께, 원캡이
채운 카드리딩·PIN 필드가 **응답으로도 디스크로도 나가지 않는다.**

## 착수 전 전제 (2026-09-04 코드 확인 완료)

1. **`E07`은 비어 있다.** 사용 중인 `E` 코드는 `E01`~`E06`(`PosResultCodeMapper`), `E40`/`E41`
   (`PosRequestTelegram`/`PosUnknownTransactionErrorResponse`), `E99`뿐이다.
2. **응답은 한 지점으로 수렴한다.** `PaymentOrchestrator.ProcessAsync`가 전문 종별 분기를 거쳐
   `PosResponseTelegram` 한 개로 모으고, 거기서 "거래 확정" 로그를 한 번만 남긴다(P22-6). 저장(P26-2)은
   이 지점에 붙인다.
3. **응답 생성 경로는 둘뿐이다.** `PosResponseTelegram.Relay`(VAN 바이트 그대로)와
   `PosResponseTelegram.BuildFailure`(요청 Clone + `#3`/`#6`/`#7`/`#8` 덮어쓰기). 카드필드 삭제는 이 둘을
   모두 지나야 한다 — 아래 "★ 착수 전 사전 조사에서 발견한 유출 1건" 참고.
4. **메모리 클리어는 이미 갖춰져 있다**(Phase 25). 요청 본문은 `TransactionQueue`의 `finally`에서
   `item.Request.Telegram.ClearBody()`, 응답 본문은 `PosSocketServer`가 프레임 송신 후
   `response.Telegram.ClearBody()`, 카드 리딩 임시 버퍼는 `PaymentOrchestrator`의 각 `finally`에서
   `SecureClear.Clear()`. **이 Phase는 그 시점들을 건드리지 않는다** — 저장은 클리어보다 앞선 지점에서
   복사본을 뜨고 그 복사본을 즉시 지운다.
...
### ★ 착수 전 사전 조사에서 발견한 유출 1건 (현재 코드의 결함)

`PosResponseTelegram.BuildFailure`는 **요청 전문을 `Clone()`해서** 실패 응답을 만든다. Phase 18에서
`#51`(PIN)만 지우도록 고쳤고(H-1), **`#45`/`#46`/`#53`은 지우지 않는다.**

따라서 **카드 리딩과 필드 채움까지 끝난 뒤 실패하는 경로**에서 암호화된 카드정보(`#46`, 196바이트)와
EMV DATA(`#53`, 604바이트)가 실패 응답에 실려 POS로 나간다. 해당 경로는 최소 둘이다.

- VAN DLL 통신 실패(`D0x`) — `RelayToVanAsync`의 `CommunicationFailure` 분기
- 필드 채움/거래 처리 중 예외(`E99`) — `TransactionQueue`의 catch가 만드는 fallback 실패 응답

**메모리 잔존 문제가 아니다**(전제 4대로 지워진다) — 지우기 **전에** 소켓으로 나가는 것이 문제다.
`#51`만 이 관점으로 걸러졌고 나머지는 누락됐다. **P26-1에서 함께 고친다**(2026-09-04 사용자 확정).

## 이 Phase에서 손대지 않는 것 (범위 밖)

- **기존 relay/실패 판정 로직** — 어떤 응답코드를 언제 내는지는 Phase 15~21에서 실장비로 검증이 끝났다.
  이 Phase는 "나가는 바이트에서 4개 필드를 지운다"와 "저장한다"만 얹는다.
- **Phase 25의 클리어 시점** — 전제 4. 새 클리어 지점을 추가하되 기존 지점을 옮기지 않는다.
- **`#4` 거래구분 코드 실제 채번** — 발주처 협의 대기(PRD §3.4.8). 상수 1곳으로 격리하고 임시값으로
  구현·검증한다.
- **VAN 측 조회/취소 API** — `FNAISCRDVAN`에 그런 기능이 없다(PRD §3.4.1). 원캡이 저장한 결과로만 복구한다.

---

## P26-1. 응답 전문의 카드리딩 필드 삭제 (PRD §4.13)

**이것을 먼저 한다.** P26-2가 저장할 바이트가 여기서 확정되기 때문이다 — 순서를 뒤집으면 카드정보가
디스크에 들어갔다가 나중에 빠지는 중간 상태가 생긴다.

### 구현할 것

- `Protocol/Pos/`에 **길이 보존 삭제**를 넣는다. 위치는 `PosResponseTelegram`(2026-09-04 사용자 확정) —
  `Relay`와 `BuildFailure` 두 팩토리가 모이는 유일한 곳이고, `#51`을 이미 여기서 지우고 있어 선례가 있다.
  기존 `EncryptedPinFieldNumber` 상수 처리를 **4개 필드 공통 처리로 확장**하는 형태가 된다.
- 대상은 `#45`(복호화 정보) / `#46`(암호화된 카드정보) / `#51`(암호화된 비밀번호 정보) / `#53`(EMV DATA).
  **필드 번호만 상수로 두고 POSITION·길이는 스키마(`PosTelegramSchema`)에서 가져온다** — 오프셋을
  하드코딩하지 않는다(계층 규칙 및 P17 스키마 자체검증의 이점을 그대로 살린다).
- **번호만으로 판단하면 안 된다 — 반드시 전문 종류로도 걸러야 한다(체크포인트 1에서 실제로 발견한
  회귀).** 애초 전제("`501008`/`800000`에는 아예 없다")는 `800000`에는 맞지만 `501008`에는 틀렸다 —
  `501008`(`NoticeInquirySchema`) 스키마에도 `#45`/`#46`/`#51`/`#53` 번호가 존재하며, 카드리딩과 전혀
  무관한 DigitalBudget 소유 업무 데이터(`#45` 납부 금액 수정 허용 유무 / `#46` 연대 납부 대상 유무 /
  `#51` 기 납부 금액 / `#53` 분야(기능) 코드)를 담고 있다. **거래구분 코드가 `902614`일 때만** 이
  삭제를 적용한다(스키마에 필드가 있는지는 그다음에 본다) — 그렇지 않으면 `501008`의 정상 응답
  데이터를 침범한다.
- **채우는 값은 space(0x20)**, 길이 유지. `PosField.Pad`가 빈 문자열에 대해 타입과 무관하게 전체 space로
  채우는 기존 동작(P17 체크포인트1 M-1)을 그대로 쓴다. **`0x00`은 절대 쓰지 않는다** — 유효한 전문은
  space/`'0'`로만 패딩된다는 원칙이자, `VanService`의 H-1 NUL 검사를 깨뜨린다.
- **`Relay` 경로에도 적용한다.** VAN이 돌려준 바이트를 감싼 직후 지운다 — "VAN 응답을 해석하지 않는다"는
  §4.10 원칙은 유지된다(승인/거절 판단을 하지 않고, 원캡이 써 넣었던 자리만 되돌린다).
- **삭제 사실을 별도로 로깅하지 않는다**(2026-09-04 체크포인트 1에서 계획 정정 — 애초 이 항목은 이
  Task에 없어도 될 것을 넣었다). `Protocol/Pos/`는 `Services/Diagnostics/FileLogger`를 참조할 수 없다
  (계층 규칙, `ViewModels → Services → Protocol → Interop` 단방향) — `PosResponseTelegram`에 로깅을
  추가하면 그 자체로 위반이 된다. 기존 `TelegramLogRedactor`는 그대로 둔다 — 로그 마스킹과 전송 삭제는
  다른 축이다.

### 완료 조건

- [x] `902614` **relay 응답**의 `#45`/`#46`/`#51`/`#53`이 전부 space다(raw 바이트로 확인).
- [x] `902614` **실패 응답**(카드 채움 후 `D0x`/`E99`)의 같은 4개 필드가 전부 space다 — **위 ★ 유출
      1건이 실제로 막혔음을 증거로 남긴다**(수정 전 재현 → 수정 후 미재현).
- [x] `#43`/`#44`/`#48`/`#50`과 그 외 모든 바이트는 **한 바이트도 바뀌지 않는다**(수정 전/후 응답
      바이트 diff가 정확히 4개 필드 구간에서만 발생).
- [x] 응답 전체 길이 1500 불변, `#0`도 불변.
- [x] `501008`/`800000` 응답은 **완전히 동일**하다(회귀 없음) — 특히 `501008`의 `#45`/`#46`/`#51`/`#53`
      (카드리딩과 무관한 DigitalBudget 업무 필드)이 삭제되지 않고 그대로 남아야 한다.
- [x] **전송(POS 응답 바이트)에서는** 지운 4개 필드 값이 나가지 않는다. **로그 파일은 이 조건에서
      제외한다** — `#51`(PIN)은 2026-09-01 사용자 확정(SEED 암호화 확정 전까지 평문 유지)에 따라
      `TelegramLogRedactor`가 로그에서는 의도적으로 마스킹하지 않는다(2026-09-04 체크포인트 1에서
      확인, 기존 결정 유지로 재확정). Phase 26이 이 기존 로깅 정책을 바꾸지 않는다.

---

## P26-2. 직전 거래 응답 저장소 (PRD §7.1)

### 구현할 것

- `Services/Storage/`에 저장소를 추가한다. `ObservedIdentityStore`를 그대로 본뜬다 — 같은 DB 파일
  (`IntegrityCheckStore.DefaultDatabasePath()`), `CREATE TABLE IF NOT EXISTS`, `ON CONFLICT DO UPDATE`
  UPSERT, **모든 공개 메서드가 예외를 밖으로 던지지 않는다**(PRD §9 — DB가 잠기거나 깨져도 결제가
  죽으면 안 된다), 실패는 반환값 + `FileLogger`.
- 저장 항목: `#9` 키오스크 전문관리번호 / 원거래 거래구분 코드 / **응답 전문 원문(BLOB)** / 응답 시각.
  타임스탬프 형식은 `IntegrityCheckStore.TimestampFormat`을 재사용한다(같은 DB에 형식이 두 개면 반드시
  헷갈린다는 기존 경고 그대로).
- **1행만 유지**한다 — 고정 키(단일 행)로 UPSERT. 이력을 쌓지 않으므로 정리 정책이 없다.
  다음 거래가 덮어쓸 때까지 유지한다(조회 후 삭제하지 않는다 — PRD §3.4.7 "조회는 반복 가능").
- **저장에 넘기는 바이트는 P26-1을 이미 통과한 응답 본문**이다. 저장용 복사본은 `SecureClear`로
  즉시 지운다(`try/finally`, 운영 PRD §4.3.3의 "임시 버퍼는 즉시").
- **호출 지점은 `PaymentOrchestrator.ProcessAsync`의 수렴 지점**(전제 2) — "거래 확정" 로그와 같은 자리,
  단 **로그보다 먼저** 기록한다. `PosSocketServer`가 응답을 송신하기 전에 이미 저장이 끝나 있어야 한다
  (PRD §3.4.6 구현 제약: 기록 → 거래 종료 순서).
- **저장 실패는 거래를 실패시키지 않는다.** 응답은 그대로 나가고 경고 로그만 남긴다 — 저장은 복구
  보조 수단이지 거래 성립 조건이 아니다.

### 완료 조건

- [x] 거래 1건이 끝나면 행이 정확히 1개 존재하고, 다음 거래 후에도 여전히 1개다(값만 바뀜).
- [x] 저장된 BLOB이 **POS로 나간 응답 본문과 바이트 단위로 동일**하다.
- [x] 저장된 BLOB에 `#45`/`#46`/`#51`/`#53` 값이 없다(전부 space).
- [x] DB 파일을 읽기 전용으로 만들거나 잠근 상태에서도 **거래가 정상 완료**되고 경고만 남는다.
- [x] 저장용 복사본이 `SecureClear`로 지워진다.
- [x] 앱을 껐다 켜도 행이 남아 있다.

---

## P26-3. 상태 조회 전문 스키마 (PRD §3.4.3, §3.4.5)

### 구현할 것

- `Protocol/Pos/Schemas/`에 조회 전문 스키마를 추가하고 `PosSchemaRegistry`에 등록한다.
  등록만 하면 `ValidateAtStartup()`이 기동 시 POSITION 연속·총 길이를 자동 검증한다.
- **요청**: 공통부 `#0`~`#13`, **70바이트**, 개별부 없음. 공통부 정의는
  `Schemas/PosCommonHeader`를 재사용한다(직접 다시 적지 않는다).
- **응답**: 공통부 70 + 원거래 거래구분 코드(N6) + 원거래 응답 전문 길이(N4) + 원문(가변). 총
  `80 + 길이`(결과 없음이면 80).
- **가변 길이가 이 프로젝트의 첫 사례다.** 기존 `PosTelegramSchema`는 `totalLength` 고정을 전제로
  자체 검증하므로, 가변 꼬리를 어떻게 표현할지 결정해야 한다 — **스키마는 고정부(80)까지만 정의하고
  꼬리 원문은 스키마 밖에서 이어 붙이는 방식**을 기본으로 한다(고정 길이 검증 이점을 잃지 않는다).
  구현 중 더 나은 방법이 보이면 그 지점에서 근거를 적고 바꾼다.
- **`#4` 거래구분 코드는 미채번**(PRD §3.4.8) — 상수 1곳(`FixedTransactionType`)에 임시값을 두고,
  그 상수에 "발주처 채번 대기, 확정되면 이 값만 교체" 주석을 단다. 기존 3종과 겹치지 않는 값을 고른다.

### 완료 조건

- [x] 앱 기동 시 `ValidateAtStartup()`을 통과한다(스키마 오류가 있으면 기동 자체가 실패해야 한다).
- [x] 요청 본문이 정확히 70바이트, 결과 없음 응답이 정확히 80바이트다.
- [x] `902614` 원문을 실은 응답이 1580바이트이고, 바깥 `#0`=1580 / 안쪽 원문의 `#0`=1500으로 **서로 다른
      값이 정확히 들어간다**(PRD §3.4.5 주의 문단).
- [x] `#4` 임시값이 `501008`/`800000`/`902614`와 겹치지 않는다.

---

## P26-4. 조회 처리 경로 (PRD §3.4.6, §4.11)

### 구현할 것

- 조회 요청을 **기존 단일 Queue에 그대로 태운다**(별도 처리 경로를 만들지 않는다) — 이것이 "처리 중"
  상태를 정의하지 않아도 되는 유일한 근거다(PRD §3.4.6). `PaymentOrchestrator.ProcessAsync`의 전문 종별
  switch에 분기를 하나 더 추가하는 형태다.
- 저장된 `#9`와 요청 `#9`가 **일치할 때만** 원거래 응답을 실어 응답한다. 불일치 또는 기록 없음이면 `E07`.
- `PosPaymentResultCode`에 항목을 추가하고 **매핑은 `PosResultCodeMapper` 한 곳에만** 둔다 —
  `Services/Payment/` 안에 `"E07"` 리터럴이 등장하지 않아야 한다(P15-3/P17-4 완료 조건 그대로).
- 조회 응답의 `#7`은 **결과가 있으면 원거래 응답의 `#7`을 그대로 relay**, 없으면 `E07`.
- **조회는 카드 리딩·VAN 호출·알림창을 일절 하지 않는다.** 설정 화면 게이트(`E03`)·무결성 체크·
  키오스크 고유번호 검사(`E06`)도 지나지 않는다 — 부작용이 없어야 반복 호출이 안전하다(PRD §3.4.7).
- 조회 자체는 **저장소를 갱신하지 않는다**(직전 거래를 조회로 덮어쓰면 원본이 사라진다).

### 완료 조건

- [x] 조회 요청이 카드 리딩·VAN 호출·알림창을 **전혀** 유발하지 않는다(로그로 확인).
- [x] `#9` 일치 → 저장 원문과 **바이트 단위로 동일한** 꼬리를 실어 응답한다.
- [x] `#9` 불일치 / 기록 없음 → `#7`이 `E07`, 개별부가 `000000` + `0000`, 본문 80바이트.
- [x] 결과 없음 응답의 어느 바이트에도 `0x00`이 없다.
- [x] 같은 조회를 연속 3회 보내도 매번 같은 응답이 오고 저장소가 바뀌지 않는다.
- [x] `Services/Payment/`에 `"E07"` 리터럴이 없다(grep).
- [x] 조회가 원거래를 추월하지 않는다 — 거래 처리 중에 조회를 밀어 넣으면 원거래 완료 후에 처리된다.
...
- "직전 거래 상태 조회" 버튼 — 마지막으로 보낸 요청의 `#9`를 그대로 재사용해 조회 전문을 보낸다.
- **응답을 일부러 못 받는 시나리오** — 요청을 보낸 뒤 응답 수신 전에 연결을 강제로 끊는다.
  이 Phase의 존재 이유를 재현하는 유일한 수단이다.
- 조회 응답의 개별부 3번(원문)을 잘라 **기존 3종 파서로 그대로 파싱**해 보여준다 — PRD §3.4.5의
  "POS는 기존 파서를 재사용하면 된다"가 실제로 성립하는지 확인하는 검증이기도 하다.

### 완료 조건

- [x] 연결 강제 종료 → 재연결 → 조회로 원래 결과를 화면에 표시하는 흐름이 시뮬레이터만으로 재현된다.
- [x] 조회 응답의 꼬리를 기존 파서에 그대로 넣어 필드가 정상 파싱된다(파서를 고치지 않는다).
...
- [x] **① 정상 거래 후 조회** — 원문이 원래 응답과 바이트 단위로 동일.
- [x] **② 응답 유실 복구** — 응답 직전 연결 강제 종료 후 재연결·조회로 결과 복구(P26-5 시나리오).
- [x] **③ 다른 `#9`로 조회** → `E07`.
- [x] **④ 기록이 없는 상태(첫 실행)에서 조회** → `E07`.
- [x] **⑤ 앱 재시작 후 조회** → 여전히 성공(메모리 저장이었으면 실패했을 케이스).
- [x] **⑥ 카드/PIN 잔존 전수 검사** — 응답 프레임과 DB BLOB의 raw 바이트에서 카드번호 패턴·PIN
      값이 검출되지 않는다. Phase 21 P21-2와 **같은 방식**(가짜 부품 + raw 바이트 전수 검사)을 쓴다.
- [x] **⑦ 회귀** — 기존 `--payment-flow-test` 하네스와 `--memory-clear-test`가 전부 통과한다.
- [x] `dotnet build` 경고 0 / 오류 0.
...
Phase 22 이후의 표준 흐름(설계 Opus → 구현 → **Opus 검증 리뷰** → 수정 → 재확인)을 그대로 따른다.

- **체크포인트 1 — P26-1·P26-2 직후.** 카드필드 삭제가 relay·실패 두 경로를 모두 덮는지, 저장 바이트가
  응답 바이트와 같은지, 기존 클리어 시점을 깨지 않았는지. **이 Phase에서 가장 회귀 위험이 큰 지점**이라
  여기서 한 번 끊는다.
- **체크포인트 2 — P26-4 직후.** 조회가 Queue 순서를 지키는지, `E07` 경로가 빠짐없는지,
  `Services/Payment/` 리터럴 규칙이 유지되는지.

### 진행 기록 (2026-09-04)

**체크포인트 1(P26-1·P26-2 직후)**: Opus 검증 결과 결함 4건 — P26-2가 `PaymentOrchestrator`에
배선되지 않아 저장 바이트=응답 바이트 검증이 애초에 불가능했던 것(High), 응답 로그의 PIN 평문
노출을 새 완료 조건으로 잘못 걸었던 것(기존 2026-09-01 결정과 충돌, 사용자 확인 후 완료 조건 쪽을
정정), `Protocol/` 계층에 로깅을 요구했던 계획 항목(계층 규칙 위반, 삭제), `StubVanRelayService`의
중복 `#51` 클리어. 전부 수정 후 재검증 — `--payment-flow-test` 137/137(커밋 `60fa9b2`).

**체크포인트 2(P26-3·P26-4 직후)**: 독립 Opus 검증 결과 **확정 결함 0건**. 가변 길이 응답의 바깥/
안쪽 길이 분리, `0x00` 미혼입, `#9` echo 무침범, `Tail` Clone(저장소 원본 보호), 부작용 완전 차단
(카드리딩/VAN/알림창/저장소 갱신 전부 미도달), 정적 초기화 순서(구현 중 실제로 겪은
`TypeInitializationException`이 지금은 고쳐져 있음), `E07` 리터럴 격리를 전부 직접 재현해 확인.
관찰 1건(조회 응답이 `#6`/`#8`을 갱신하지 않음)은 PRD §3.4.5 문구를 그대로 따른 설계라 사용자 확인
후 유지 — `--payment-flow-test` 169/169(커밋 `7ae07ca`).
...
2. **착수 전 조사에서 현재 코드의 실제 결함 1건을 발견·수정** — `PosResponseTelegram.BuildFailure`가
   ...
   26 설계 작업이 아니었다면 발견되지 않았을 기존 결함이다.
3. **회귀 발견·즉시 수정** — 필드 번호만으로 카드리딩 필드를 판단하면 `501008`의 무관한 업무 필드
   (납부 금액 수정 허용 유무 등)를 침범한다는 것을 P26-1 검증 중 발견해, 거래구분 `902614` 전용
   가드로 좁혔다(다른 세션이 먼저 고쳐 이 세션이 확인·계승).
4. **Opus 검증 3라운드(체크포인트 1·2 + P26-5/P26-6 사후 검증) 전부 독립 재현** — 보고를 그대로
   믿지 않고 매번 빌드·하네스 재실행·raw 바이트 대조를 직접 수행했다. 확정 결함은 체크포인트 1의
   4건(전부 수정)과 사후 검증의 경미한 표시 결함 1건(전부 수정)뿐, 체크포인트 2는 확정 결함 0건.
5. **두 코드베이스 간 프로토콜 정합성 실증** — 본체 앱과 시뮬레이터가 소스를 전혀 공유하지 않는
   상태에서 신규 전문(999999) 필드 정의를 각각 독립적으로 옮겨 적었는데도 `#1`~`#15` 전부 정확히
   일치함을 확인 — 이 Phase가 SPEC 없이 설계한 전문이 실제로 자기 완결적임을 보여준다.

Phase 26으로 2차 범위(payment_relay)에 남아 있던 마지막 설계 항목(소켓 통신 장애 시 응답 복구)이
닫힌다. 남은 것은 발주처 협의·실서버 대기 항목뿐이다(아래 "열린 항목").

---

## 완료 기준 (Phase 전체)

1. P26-1~P26-6의 모든 완료 조건 통과.
2. 응답 유실 시나리오에서 POS가 **재전송 없이** 원래 결과를 복구할 수 있음이 시뮬레이터로 확인된다.
3. `902614`의 응답·저장 어디에도 `#45`/`#46`/`#51`/`#53` 값이 남지 않는다.
4. 기존 3전문의 동작이 바뀌지 않는다(회귀 없음).
5. 아래 "열린 항목"이 문서에 정확히 기록된다.

## 열린 항목 (이 Phase로 닫히지 않는 것 — 2026-09-04 기준 여전히 유효)

- **`#4` 거래구분 코드 채번** — 발주처 협의. 상수 1곳 교체로 끝난다.
- **SPEC 개정** — 이 전문은 POS-원캡 전산설계서에 아직 없다. §4.13(응답에서 4개 필드 삭제)도 함께
  확인받는다.
- **`#53 EMV DATA`의 응답 방향 값 유무** — 인터넷지로도 SET하는 공유 필드라, 인터넷지로가 응답에 값을
  실어 보낸다면 POS가 그 값을 못 받게 된다. 발주처 확인 또는 실서버 실측으로 확정하고, 값이 있으면 이
  필드만 삭제 대상에서 뺀다.
- **`nRet==0` 실서버 relay** — Phase 20/21에서 이어지는 서버 대기 항목. 조회로 되돌려주는 원문이 실제
  VAN 응답인 경우는 서버가 붙어야 검증된다.

## 착수 순서 요약

P26-1(삭제) → P26-2(저장) → **체크포인트 1** → P26-3(스키마) → P26-4(조회 경로) → **체크포인트 2** →
P26-5(시뮬레이터) → P26-6(검증).

**P26-1을 P26-2보다 먼저** 하는 것이 이 순서의 핵심이다 — 저장할 바이트가 먼저 확정돼야 카드정보가
디스크에 들어갔다 빠지는 중간 상태가 생기지 않는다.

---

# Phase 29 실행계획서 — 국세 결제 화면 1단계 (원캡 단독 결제)

> **근거**: `PRD.md` §12(결제 화면 요구사항), §3.3.1(2026-09-15 개정판 변경점), §3.3.2(연쇄 범위 밖),
> §4.11 하단(`031` 처리) · `ROADMAP.md` Phase 29 · `spec_open_questions.md`(연쇄 조사·질의 대장)

**이 Phase가 끝나면**: 홈 화면 "결제" 카드에서 결제 화면이 열리고, `501008`/`800000`/`902614` 세 전문을
각각 소켓(`localhost:8002`)으로 보내 응답을 받아 필드 단위로 볼 수 있다. 전 필드는 임의값이며, **원캡이
카드리딩으로 채우는 필드가 공백에서 실제 카드 데이터로 덮인 것이 화면에서 확인된다.**

## 이 Phase의 성격 (2026-09-18 사용자 확정)

이 화면은 **테스트 전용이 아니라 원캡 단독 결제 화면**이다 — 키오스크가 없는 현장에서도 국세 결제가
가능해야 한다. 지금까지 원캡은 POS가 보내온 전문을 처리하는 수동적 중계자였고, 이 화면이 원캡이 스스로
결제를 시작하는 첫 경로다.

**다만 이번 단계는 전 필드 임의값이다.** 전문 간 필드 연쇄(앞 전문 응답 → 뒤 전문 요청)는 SPEC 근거가
확정되지 않아 범위 밖으로 뒀다(PRD §3.3.2) — `501008`→`902614`는 명문이 있으나 절삭·표현 변환·출처
미지정이 섞여 있고, `800000`→`902614`는 명문이 아예 없다. **지금 매핑을 코드에 넣으면 우리 추정이
사실상 정본이 된다.** 실제 값으로 채우는 것은 `spec_open_questions.md` Q1~Q11이 해소된 뒤다.

이 결정 덕분에 이번 Phase가 증명하는 것이 분명해진다 — **전문 왕복, `#0` 길이 프레이밍, POSITION
정합성, CP949 인코딩·패딩, 원캡 담당 필드 채움**. 모두 연쇄와 무관하게 이미 확정된 계약이다.

## 착수 전 전제 (2026-09-18 코드 확인 완료)

1. 3전문 스키마는 이미 있고 **2026-09-15 개정판에서도 필드 구성이 바뀌지 않았다** — `Protocol/Pos/Schemas/`
   (`NoticeInquirySchema` 706 / `CardInfoInquirySchema` 500 / `CardApprovalSchema` 1500)를 손대지 않는다.
2. `PosTelegramSchema`는 생성자에서 POSITION 연속성과 총 길이를 자기 검증하고, `FieldsOwnedByOneCap()`으로
   원캡 담당 필드를 돌려준다 — 임의값 생성기가 제외 목록을 새로 하드코딩할 필요가 없다.
3. `PosField.Pad()`가 패딩을 담당한다(`N`=우측정렬 `'0'`, 그 외=좌측정렬 space, 초과 시 예외). 생성기는
   **원시값만** 만든다.
4. 프레이밍은 `PosMessageFramer`와 `PosMessageEncoding.Value`(CP949)가 이미 담당한다.
   `Services/Diagnostics/PosClientTestScenarios.cs`에 소켓 클라이언트 쪽 프레이밍 예시가 있다(`#0` 4바이트
   길이 + 본문).
5. 결제 Flow는 `Services/Payment/PaymentOrchestrator.cs`가 전문 종별로 분기하고
   (`HandleNoticeInquiryAsync` / `HandleCardInfoInquiryAsync` / `HandleCardApprovalAsync`), 카드리딩 필드
   채움은 `FillCardApprovalFields`에 있다 — **이 Phase에서 오케스트레이터는 수정하지 않는다.**
6. `App.xaml.cs`는 여전히 `StubVanRelayService`를 물린다(의도된 기동 WARN 포함). 이번 Phase도 그대로 둔다.
7. 홈 화면의 "결제" 카드는 `ViewModels/HomeViewModel.cs`의 `OpenTrans()`가
   `NotImplementedCardRequested`를 올려 MessageBox로 끝난다 — 이 지점이 연결부다.
8. `Views/SetupScreenGate.cs`가 설정 화면 열림을 판단해 `E03`을 만든다 — **이 화면은 등록하지 않는다**
   (등록하면 자기 창 때문에 자기 요청이 거절된다, PRD §12.4).

## 이 Phase에서 손대지 않는 것 (범위 밖)

- **전문 간 필드 연쇄** — SPEC 확정 후 별도 Phase(PRD §3.3.2, `spec_open_questions.md`).
- **`999999` 거래 상태 조회 전문** — 화면에 넣지 않는다.
- **실 VAN 연동** — `StubVanRelayService` 유지. `App.xaml.cs`의 VAN 배선을 바꾸지 않는다.
- **`PaymentOrchestrator`·`PosSocketServer`·3전문 스키마** — 읽기만 하고 수정하지 않는다.
- **결제 알림창(§5)** — 카드리딩·PIN 안내는 기존 알림창이 담당한다. 같은 안내를 이 화면에 만들지 않는다.
- **인터넷지로 API 3종**(`avail-card`/`card-installment`/`card-fee`) — 원캡이 붙는 대상이 아니다.

## 체크포인트 (Opus 리뷰 지점)

| 체크포인트 | Task | 성격 |
|---|---|---|
| — | P29-1 | 문서 전용(이미 완료). 별도 리뷰 없이 진행 |
| — | P29-2·P29-3 | 응답코드 대조 + `031` 로그 설명. 코드 변경 범위가 좁고(`Describe` 추가뿐) 기존 relay·알림
  경로를 건드리지 않아 별도 리뷰 없이 진행 |
| **CP1** | P29-4·P29-5 | **신규 계층(임의값 생성기 + 소켓 클라이언트) 완성.** 아직 화면에 안 붙어 있어
  기존 결제 경로에 기능적 영향은 없지만, **여기서 잘못되면 화면 작업 내내 "값이 틀렸는지 화면이
  틀렸는지" 구분이 안 된다** — CP1의 존재 이유 |
| **CP2 ★** | P29-6·P29-7 | **결제 화면을 실제로 배선.** 홈 화면 연결(`OpenTrans()` 교체)과
  `SetupScreenGate` 열외가 여기 있다 — 기존 설정 화면 게이트 동작에 회귀가 생기면 가장 위험한 지점 |
| — | P29-8 | 실측 검증 · 회귀 · 문서 갱신. 별도 리뷰 대신 회귀 확인으로 갈음 |

## 구현 담당 (메인 세션 vs `csharp-wpf-developer` 에이전트)

기준 두 가지, 둘 중 하나만 걸려도 메인 세션이 직접 한다:

1. **왕복 조정 필요 여부** — "구현→빌드→검증"이 자기완결적인 작업은 에이전트, 사용자와 스크린샷을 보며
   여러 라운드 실시간으로 맞춰야 하는 작업(값 튜닝, 레이아웃 미세조정)은 메인 세션. 에이전트를 거치면
   라운드마다 "호출→요약 보고→사용자 피드백→재호출"이 끼어 느려지고 뉘앙스가 손실된다.
2. **대화 맥락 의존 여부** — fork가 아닌 일반 에이전트는 호출될 때마다 완전히 새로 시작해 이 대화에서만
   존재하는 맥락(문서화되지 않은 결정 이력, "이전 버전으로 되돌려" 같은 누적된 판단)을 전혀 모른다.
   필요한 맥락이 코드/문서에 이미 다 있는 자기완결적 작업은 에이전트에 적합하지만, 대화에서만 쌓인
   맥락을 계속 참조해야 하는 작업은 그 맥락을 프롬프트에 옮겨 적는 비용이 크면 메인 세션이 낫다(옮겨
   적다 빠뜨리면 에이전트가 이미 뒤집힌 예전 결정으로 돌아가는 위험도 있다).

(2026-09-21 사용자 확인). 이 구분을 뒤늦게라도 문서화한다.

| Task | 담당 | 근거 |
|---|---|---|
| P29-1 | 메인 세션 | 문서 전용, 반복 조정 없음 |
| P29-2 | 메인 세션 | SPEC 대조는 읽기·판단 작업이라 `pos-onecap-spec-expert`(읽기 전용) 조회는 위임하되 최종 대조·기록은 메인 세션 |
| P29-3 | 메인 세션 | `Describe` 문자열 추가 1건, 왕복 불필요 |
| P29-4 | `csharp-wpf-developer` | Protocol 계층 신규 생성기 + 단위 검증(`PosField.Pad` 예외 케이스)이 자기완결적 — 화면이 없어 사용자 시각 피드백 라운드가 없음 |
| P29-5 | `csharp-wpf-developer` | 소켓 클라이언트 신규 계층, 기존 하네스 패턴 재사용 검증까지 자기완결적 |
| P29-6 | **메인 세션 (실제로도 그랬음)** | 최초 화면 구현은 에이전트로 가능하지만, **compact 모드 대응·줄간격 튜닝**은 스크린샷마다 사용자가 즉시 피드백을 주는 반복 조정이라 메인 세션이 직접 진행했다 — 에이전트로 했다면 각 라운드가 더 느리고 오해도 더 컸을 것 |
| P29-7 | `csharp-wpf-developer` | 기존 `ReaderSetupWindow`/`ShopSetupWindow` 패턴을 그대로 따르는 배선 작업 — 패턴이 이미 확정돼 있어 왕복 조정이 필요 없음. **단, `SetupScreenGate` 열외 판단은 위험도가 높으니 구현 후 메인 세션에서 diff를 직접 재확인한다** |
| 체크포인트 1·2 | `checkpoint-reviewer` (Opus 고정) | 구현자의 자기 보고를 믿지 않는 것이 존재 이유 — 항상 별도 세션 |
| P29-8 | 메인 세션 + 사용자 | 실기 조작이 필요해 UIPI로 에이전트가 접근 불가한 구간이 있음(`feedback_elevated_process_handling`) — 사용자 직접 조작 요청이 필요할 수 있음 |

Phase 30 배분(같은 기준 적용):

| Task | 담당 | 근거 |
|---|---|---|
| P30-1 | `csharp-wpf-developer` | 스텁 서비스 1개 파일 확장 + 기존 회귀 하네스 2종 재실행까지 자기완결적. 화면 무관이라 시각 피드백 라운드 없음 |
| P30-2 | 메인 세션 | 매핑표를 코드로 옮기는 작업인데 **출처가 PRD §3.3.2이고 그 근거 이력(세 번 바뀐 `#38`, 표기 오타였던 `*17` 등)이 이 대화에 쌓여 있다** — 기준 2(맥락 의존). 숫자 하나 잘못 옮기면 전문이 통째로 틀어지므로 직접 한다 |
| P30-3 | `csharp-wpf-developer` | 순수 로직 + 단위 검증(절삭 경계 케이스)이 자기완결적. CP949 반글자 규칙은 이 계획서에 명시돼 있어 맥락 의존 없음 |
| P30-4 | `csharp-wpf-developer` | ViewModel 배선 작업이고 완료 조건이 명확. **단, "원캡 중계 경로 3개 파일 불변" 제약은 위험도가 높으니 구현 후 메인 세션이 `git diff`를 직접 재확인한다** |
| P30-5 | `csharp-wpf-developer` | 재생성 대상 필터 1곳 추가, 자기완결적 |
| P30-6 | **메인 세션** | 색 선택·compact 모드 레이아웃은 P29-6과 같은 성격의 반복 시각 조정이라 기준 1에 걸린다(P29-6에서 `PaymentRequestCardWidth` 튜닝에 여러 라운드가 든 전례) |
| 체크포인트 1·2 | `checkpoint-reviewer` (Opus 고정) | 같은 이유 |
| P30-7 | 메인 세션 + 사용자 | 실기 카드·PIN 조작 필요, UIPI로 에이전트 접근 불가 |

---

## P29-1. SPEC 정본 교체 + 질의 대장 신설 (PRD §3.3.1, §3.3.2)

### 구현할 것

- `docs/payment_relay/` 전체의 SPEC 정본 경로를 `_20260915 (1).pdf`로 갱신한다. **과거 시점을 서술하는
  역사적 언급은 그대로 둔다**(Phase 17/20 기록, `§10.1` 확정 사항 표의 날짜 있는 행 — 이 저장소 규칙).
- PRD `§3.3.1`에 개정 내용을 기록한다: 999999 등재 / 원캡 응답코드 섹션 신설 / `031` 추가 /
  `#38` kiosk ○ 추가 / **3전문 필드 구성은 불변**.
- PRD `§3.4.9`(발주처 제안 초안)에 "반영 완료" 표시를 남기되 제안 당시 서술은 보존한다.
- **`spec_open_questions.md` 신설** — 연쇄 조사 기록(501008→902614 19행, 800000→902614 후보 4행)과
  발주처 질의 Q1~Q11. 근거 등급 (가)/(나)/(다)/(라)를 엄격히 표기한다. **PRD에 이 표를 복제하지 않는다.**
- PRD `§12` 신설, `ROADMAP.md` Phase 29 절·진행 상태 표·미확정 사항·참고 문서 갱신.

### 완료 조건

- [x] PRD `§3.3` 정본 경로가 `_20260915 (1).pdf`이고 `§3.3.1`에 개정 4건이 적혀 있다.
- [x] PRD `§3.3.2`가 연쇄를 범위 밖으로 선언하고 `spec_open_questions.md`를 가리킨다.
- [x] `spec_open_questions.md`가 존재하고 Q1~Q11 전부에 근거 페이지가 있다.
- [x] PRD `§12`에 화면 성격·임의값 규칙·전송 경로·화면 경합이 적혀 있다.
- [x] PRD `§4.11` 하단에 `031` 처리 판단과 근거가 적혀 있다.
- [x] `ROADMAP.md`에 Phase 29 행과 본문이 있고, 미확정 사항에 연쇄 항목(8번)이 추가됐다.
- [x] `ROADMAP.md` 참고 문서 절의 정본 경로가 갱신되고 `spec_open_questions.md`가 등재됐다.

## P29-2. 원캡 응답코드 SPEC vs 코드 대조 (PRD §4.11)

### 구현할 것

- SPEC p.22~23 "2) 원캡 응답 코드"의 `E01~E07`/`E40~E43`/`E99`, `R00~R23`/`R24~R32`, `D01`/`D02`를
  `Services/Payment/PosResultCodeMapper.cs`, `Services/Diagnostics/LogCodeCatalog.cs`,
  `Services/Diagnostics/InternalFaultCodes.cs`와 **코드값 단위로 1:1 대조**한다.
- 불일치는 표로 남기고 **"코드가 맞고 SPEC이 틀림" / "SPEC이 맞고 코드가 틀림"을 구분**한다 — 이 체계는
  원캡이 정의해 발주처에 제공한 것이므로(PRD §10) **코드가 정답일 가능성이 있다.** 어느 쪽을 고칠지는
  사용자에게 보고한 뒤 결정한다.
- 대조 시 SPEC 확인은 `pos-onecap-spec-expert`에 위임한다(공통 규칙 3).
...
- `031`은 서버가 판단해 `#7`에 싣는 값이고 원캡은 relay만 한다(§4.10) — **분기를 추가하지 않는다.**
- `Services/Diagnostics/LogCodeCatalog.cs`의 `Describe`에 `031` 설명을 추가해 로그에서 뜻이 읽히게 한다.
  기존 코드 설명 추가 방식(`FileLogSink.BuildCodeNoteBytes`가 code를 가진 레코드에 설명 줄을 붙인다)을
  그대로 쓴다. **이게 이 Task의 유일한 코드 변경이다.**
- **장애 알림 대상 제외는 이미 확정됐다**(2026-09-18 사용자 확정, `fault_alert_catalog.md` §2.1 갱신 완료,
  PRD §4.11 하단). 판정 주체가 다르기 때문이다 — `E40`/`E41`은 원캡이 채운 코드고 `031`은 서버가 보낸
  값의 중계다. `FaultAlertJudge.Classify`가 원캡 코드만 분기하고 나머지는 fall-through로 침묵하므로
...
- `Protocol/Pos/`에 임의값 생성기를 추가한다. 입력은 `PosTelegramSchema`, 출력은 필드번호→원시값.
- **표현·길이만 본다.** 필드 이름이나 업무 의미로 값을 바꾸지 않는다 — 그렇게 하면 PRD §3.3.2가 피하려는
  추정이 코드로 스며든다.
- 표현별 문자 집합: `N`=숫자, `AN`=영숫자, `AHN`=한글 포함, `ANS`/`AHNS`=특수문자 포함
  (`Protocol/Pos/PosFieldType.cs`의 정의를 따른다).
- **길이는 바이트 기준이고 인코딩이 CP949라 한글은 1자당 2바이트다.** 문자 수로 세면 `PosField.Pad()`가
  초과 예외를 던진다 — 이걸 **일부러 테스트로 확인**해 규칙을 못 박는다(홀수 길이 AHN 필드에서 1바이트가
  남는 경계도 함께).
- 패딩은 `PosField.Pad()`에 맡기고 생성기는 원시값만 만든다(패딩 규칙을 두 곳에 두지 않는다).
- **`schema.FieldsOwnedByOneCap()`에 속한 필드는 생성하지 않는다** — 공백으로 남긴다.
- 계층 규칙 준수: 이 코드는 `Protocol` 계층이므로 WPF 타입이나 `Services`를 참조하지 않는다.

### 구현 결과 (2026-09-18)

`Protocol/Pos/PosRandomValueGenerator.cs`(`GenerateRandomRequest`/`GenerateValue`)와
`Services/Diagnostics/PosRandomValueGeneratorSelfTest.cs`(`--random-value-generator-test`)를 추가했다.
ASCII 계열(N/A/AN/ANS)은 문자 수=바이트 수라 그대로 채우고, 한글이 섞이는 AHN/AHNS는 남은 바이트 예산을
보며 매 글자마다 한글(2바이트)/ASCII(1바이트) 중 무작위로 골라 예산을 절대 넘기지 않는다.

실제 앱(`dotnet build` 경고 0/오류 0)을 `--random-value-generator-test`로 직접 실행해 로그로 확인:

```
[501008] 필드 56개 생성 완료 — 본문 706바이트, 원캡 담당 0개 공백 유지 확인
[800000] 필드 28개 생성 완료 — 본문 500바이트, 원캡 담당 1개 공백 유지 확인
[902614] 필드 54개 생성 완료 — 본문 1500바이트, 원캡 담당 8개 공백 유지 확인
홀수 길이(7바이트) AHN 필드 100회 생성 — 전부 Pad 예외 없이 통과
완료 — 실제 3전문=통과, 홀수길이 한글 경계=통과, N필드 숫자전용=통과, 종합=통과
```

부수 발견 — `CardApprovalSchema.cs`의 클래스 주석이 "원캡 담당 필드는 7개"로 stale하게 남아 있었다
(실제 `FieldsOwnedByOneCap()`는 이미 8개를 정확히 반환, #51 포함 등록도 코드에 이미 있었음 — 주석만
2026-08-26 이전 상태로 멈춰 있었다). 8개로 정정했다.

### 재확정 (2026-09-18, P29-6 UI 재작업 중 사용자 지시로 함께 개정)

화면을 "kiosk 기준"으로 다시 짜면서(아래 P29-6 재작업 참고), 생성 대상 기준도 "원캡 담당만 제외"에서
**"kiosk가 SET 장소에 없는 필드는 전부 제외"**로 넓혔다 — 인터넷지로/VAN 단독 담당 필드(예:
`#7 응답 코드`)에도 여전히 임의값이 들어가고 있었는데, 실제 kiosk라면 그런 자리는 애초에 안 채우기
때문이다. `GenerateRandomRequest`의 제외 판정을 `schema.FieldsOwnedByOneCap()` 기반에서
`!field.Owners.HasFlag(PosFieldOwner.Kiosk)` 기반으로 바꿨다.

재검증(`--random-value-generator-test`, `dotnet build` 경고 0/오류 0 상태):

```
[501008] 필드 56개 중 kiosk 담당 13개 생성, 비담당 43개 공백 유지 확인 — 본문 706바이트
[800000] 필드 28개 중 kiosk 담당 11개 생성, 비담당 17개 공백 유지 확인 — 본문 500바이트
[902614] 필드 54개 중 kiosk 담당 38개 생성, 비담당 16개 공백 유지 확인 — 본문 1500바이트
홀수 길이(7바이트) AHN 필드 100회 생성 — 전부 Pad 예외 없이 통과
완료 — 실제 3전문=통과, 홀수길이 한글 경계=통과, N필드 숫자전용=통과, 종합=통과
```

### 데이터 오류 정정 (2026-09-18, 사용자가 화면을 보고 지적 → `pos-onecap-spec-expert` 재확인)

사용자가 501008 요청 패널에서 `#5 상태 코드`·`#13 FILLER`가 왜 뜨는지 물었다. `NoticeInquirySchema.cs`의
Owners가 이 둘에 `Kiosk`를 잘못 붙여둔 상태였다 — SPEC p.8(501008 전용 SET 장소표)을 재확인한 결과 둘 다
kiosk 칸이 빈칸이고(디지털예산·인터넷지로만 체크), p.5의 "공통부분 일반표"(인터넷지로/kiosk 2열짜리,
501008 전용표와는 다른 표)와 혼동해 옮겨 적은 것으로 추정된다. `#5`/`#13`에서 `Kiosk`를 뺐다.

같은 패턴이 다른 전문에도 있는지 902614의 `#5`도 별도로 재확인했다 — **902614는 SPEC 원문 자체가
`#5`에 kiosk ○를 포함**하고 있어(p.14) 코드가 맞다, 수정하지 않았다. 세 전문의 SET 장소표는 서로
독립적으로 정의돼 있어 "상태 코드는 kiosk가 안 채운다" 같은 규칙이 전문을 가로질러 일반화되지 않는다
(CLAUDE.md의 기존 경고 — SET 장소 열은 전문마다 직접 대조해야 한다).

재재검증(`--random-value-generator-test`):

```
[501008] 필드 56개 중 kiosk 담당 11개 생성, 비담당 45개 공백 유지 확인 — 본문 706바이트
[800000] 필드 28개 중 kiosk 담당 11개 생성, 비담당 17개 공백 유지 확인 — 본문 500바이트(변경 없음)
[902614] 필드 54개 중 kiosk 담당 38개 생성, 비담당 16개 공백 유지 확인 — 본문 1500바이트(변경 없음)
```

`--payment-screen-test`로 501008 탭을 다시 확인 — 요청 패널에 정확히 11개 필드(#1/#2/#3/#4/#6/#8/#9/
#11/#12/#14/#16)만 보이고 `#5`/`#13`은 사라졌다(스크린샷: `payment_screen_ownership_fix.png`).

### 완료 조건

- [x] 3전문 전부에 대해 생성 → `PosField.Pad()` → `ToBody()`가 예외 없이 통과하고, 본문 길이가
      스키마 총 길이(706/500/1500)와 정확히 일치한다.
- [x] 생성된 본문에 `0x00`이 없다(§4.10 금지 규칙과 같은 기준).
- [x] `N` 필드는 숫자만, `AHN` 필드는 CP949 인코딩 후 길이가 선언 길이를 넘지 않는다.
- [x] **(2026-09-18 재확정) kiosk가 SET 장소에 없는 필드는 전부 공백이다** — 원캡 담당(`800000 #14`,
      `902614` 8개)뿐 아니라 인터넷지로/VAN 단독 담당 필드도 포함(위 재검증 로그로 확인).
- [x] 한글 바이트 길이 경계 테스트가 있고 통과한다(합성 7바이트 AHN 필드, 100회).

## P29-5. POS 역할 소켓 클라이언트 (PRD §12.4)

### 구현할 것

- 루프백 소켓 클라이언트를 재사용 가능한 서비스로 만든다(`Services/Pos/` 아래). 연결 → 프레임 전송 →
  응답 프레임 수신 → 해제.
- `Services/Diagnostics/PosClientTestScenarios.cs`의 프레이밍·응답 읽기 패턴을 **재사용**한다(새로 설계
  하지 않는다, 공통 규칙 4). `PosMessageFramer`/`PosMessageEncoding`을 그대로 쓴다.
- `KioskSim/Net/OneCapClient.cs`는 **별도 프로젝트라 참조하지 않는다** — 패턴만 참고한다.
- 타임아웃은 `PosSocketServer`의 기존 값(쓰기 5초, 응답 후 유휴 읽기 10초)과 어긋나지 않게 잡되,
  `902614`는 카드리딩·PIN 때문에 **응답까지 오래 걸린다**(결제 데드라인 기본 120초, §4.9) — 클라이언트
  읽기 타임아웃을 그보다 넉넉히 둔다. 이 값을 임의로 정하지 말고 `ShopSettings.CardReadTimeoutSeconds`
  기반으로 계산한다.
- UI 스레드를 막지 않는다(`async`), 취소 가능하게 만든다.

### 구현 결과 (2026-09-18)

`Services/Pos/PosClient.cs`(`ConnectAsync`/`SendAsync`, `IDisposable`)를 추가했다. `PosMessageFramer.
BuildFrame`/`Append`를 그대로 재사용하고(새 프레이밍 설계 없음), 연결·응답 각각 `Task.WhenAny(작업,
Task.Delay)` 레이스로 타임아웃을 건다(`PaymentOrchestrator`의 기존 패턴과 동일 — P21 참고). 타임아웃이
나면 소켓을 강제로 닫아 대기 중이던 읽기를 확실히 끝내고(.NET Framework의 `NetworkStream.ReadAsync`는
취소 토큰만으로는 실제 I/O를 못 멈추는 경우가 있어서), 그 인스턴스는 폐기 대상으로 문서화했다(재사용
불가, 새 인스턴스로 재시도).

- 501008 기본 타임아웃(`DefaultResponseTimeout`=10초)은 서버의 유휴 타임아웃(10초)과 대칭으로 잡았다.
- 902614 타임아웃(`ComputeCardApprovalResponseTimeout`)은 `ShopSettings.CardReadTimeoutSeconds`+30초로
  계산한다(임의로 정하지 않음).
- 생성자에 `port` 파라미터(기본 8002)를 추가해 셀프테스트가 가짜 리스너에 붙을 수 있게 했다 — 실제
  배선은 항상 기본값을 쓴다.

검증(둘 다 `dotnet build` 경고 0/오류 0 상태에서 실행, 로그로 확인):

1. **`--pos-client-test`(Scenario9 신설)** — 기존 8개 시나리오(원시 소켓)에 이어 `PosClient` 자체로
   501008을 실제 서버(자기 자신, `localhost:8002`)에 보내 왕복 확인:
   `[9] 완료 — 응답 수신(길이=706바이트, 기대=706바이트)`. 기존 시나리오 1~8도 `ERROR` 0건으로 회귀 없음.
2. **`--pos-client-resilience-test`(신설)** — 실 서버 없이 확인 가능한 두 가지:
   - 902614 타임아웃 여유: `CardReadTimeoutSeconds`가 30/60/120/300초일 때 전부 클라이언트 타임아웃이
     더 크다는 것을 구조적으로 확인(실제로 150초 기다리지 않음) — "타임아웃 여유=통과".
   - 서버 무응답 종료 내성: 가짜 `TcpListener`가 연결만 받고 응답 없이 닫으면 `SendAsync`가
     `IOException`을 던짐(프로세스 죽지 않음, catch 가능) — "무응답 종료 내성=통과".

UI 스레드 블로킹 확인은 이 Task 단계에서 UI가 아직 없어(P29-6에서 화면이 생긴다) 구조적으로만
확인했다(`async`/`Task` 기반 API, 동기 블로킹 호출 없음) — 실측(창이 실제로 응답하는지)은 P29-6
완료 조건으로 넘긴다.

### 완료 조건

- [x] `501008` 1건 전송 → 응답 프레임 수신이 성공한다(리더기 불필요) — Scenario9로 실측.
- [x] `902614` 전송 시 카드리딩·PIN을 기다리는 동안 클라이언트가 타임아웃으로 먼저 끊지 않는다 —
      구조적 확인(타임아웃 계산식이 항상 서버 데드라인보다 큼), 실제 카드 왕복은 P29-8(실기 검증)에서.
- [x] 서버가 응답 없이 연결을 닫는 경우에도 클라이언트가 예외로 죽지 않고 오류를 돌려준다 — 가짜
      리스너로 실측.
- [x] UI 스레드 블로킹이 없다(전송 중 창이 응답한다) — **구조적 확인만(async API)**, 실측은 P29-6으로
      이월.

## 체크포인트 1 — Opus 검증 리뷰 (P29-4·P29-5 직후)

**여기서 한 번 멈춘다.** 임의값 생성기와 소켓 클라이언트가 이 Phase의 토대이고, 화면(P29-6)은 이
둘 위에 얹힌다 — 토대가 틀린 채로 화면을 만들면 결함이 두 계층에 걸쳐 섞여 원인 추적이 배로 든다.
리뷰 대상:

- P29-4가 만든 임의값이 3전문 전부에서 `PosField.Pad()` → `ToBody()`를 예외 없이 통과하는가(특히
  홀수 길이 `AHN` 필드의 CP949 바이트 경계)
- `schema.FieldsOwnedByOneCap()` 목록이 생성 대상에서 정확히 빠졌는가(`800000 #14`, `902614` 8개) —
  하나라도 빠뜨리면 원캡이 카드리딩으로 채운 값을 임의값이 먼저 덮어써 화면 검증 자체가 무의미해진다
- 생성기가 필드 이름/업무 의미로 값을 바꾸지 않았는가(PRD §3.3.2가 피하려는 "추정이 코드로 스며드는
  것"이 여기서 새로 생기면 안 된다)
- P29-5 소켓 클라이언트가 `902614`의 긴 응답 대기(카드리딩+PIN)를 `ShopSettings.CardReadTimeoutSeconds`
...
- `Views/`에 결제 화면 창, `ViewModels/`에 대응 ViewModel을 추가한다. **MVVM 필수**(공통 규칙 6) — 창
  핸들에 직접 묶인 코드만 코드비하인드에 남긴다.
- 전문 3종을 각각 다루는 구성(탭 또는 그에 준하는 전환)으로, 각 전문마다:
  - **요청 필드 표**: 번호·이름·표현·길이·값. 값은 편집 가능.
  - **응답 필드 표**: 같은 열 구성, 읽기 전용.
  - **원문 패널**: hex + CP949 텍스트. `#0` 길이 헤더를 포함해 보여준다.
  - `임의값 재생성` / `전송` 커맨드.
- **카드리딩 필드는 회색 읽기전용**으로 렌더한다. 전송 후 **원캡이 실제로 채운 값을 요청 원문에서 읽어
  되비친다** — 어느 필드가 덮였는지 눈으로 확인하는 것이 이 Phase 검증의 핵심이다.
- 마스킹(`TelegramLogRedactor`)은 **화면 표시에 적용하지 않는다**(원문 대조가 목적). 단 **로그로 나가는
  경로는 기존 마스킹 규칙을 그대로 따른다** — 화면에 보인 값이 로그로 새지 않게 한다.
- **응답 패널에 "VAN 스텁 — 이 응답은 요청의 에코다" 경고를 고정 표시한다**(2026-09-18 사용자 확정).
  `StubVanRelayService.BuildFakeSuccess`는 요청 전문을 clone해 **`#3`(→`0210`)·`#6`(→`C`)·`#7`(→`000`)·
  `#8`(응답 시각) 네 개만** 덮어쓰고 나머지는 요청 그대로 돌려준다 — 즉 응답 표에 보이는 업무부 값은
  **우리가 보낸 임의값이 되돌아온 것**이고 진짜 VAN이 채운 값이 아니다. 이걸 표시하지 않으면 임의값을
  실제 응답으로 오독한다. 스텁이 실제로 덮어쓴 그 4개 필드는 시각적으로 구분해 표시한다. (`902614`는
  Phase 26 P26-1이 `#45`/`#46`/`#51`/`#53`을 space로 지우므로 그 4개만 공백으로 보인다 — 이것도 주석으로
  설명한다.) **스텁 자체는 수정하지 않는다.**
- 스타일은 기존 `Themes/*.xaml` 리소스를 쓴다. `images/payment_screen.png`는 **배치(카드형 패널 + 우측
  응답 정보 패널)만 참고**하고, 그 샘플의 탭·입력 항목은 일반 VAN 거래용이라 따르지 않는다.
- 결제 알림창이 이 창 위에 뜬다 — 알림창의 `Topmost`·비노출 정책(§5.1)을 건드리지 않는다.

### 구현 결과 — 2라운드 UI 재작업 (2026-09-18)

**1라운드(원래 지시대로 구현)**: `Views/PaymentScreenWindow.xaml`, `ViewModels/Payment/`
(`PaymentScreenViewModel`/`PaymentTelegramTabViewModel`/`PosFieldRowViewModel`)을 만들어 3탭 + DataGrid형
요청/응답 표 + hex/CP949 원문 패널로 구현했다(위 "구현할 것" 그대로). `csharp-wpf-developer` 에이전트가
빌드(경고 0/오류 0) 후 스크린샷까지 확보했다.

**2라운드 — 사용자가 스크린샷을 보고 두 차례 재지시(2026-09-18, 같은 날)**:

1. *"DataGrid + hex/CP949 원문 패널은 참고 이미지와 다르다"* — `docs/home_reader_setup/screenshots/pay_screen.png`
   스타일(카드형 헤더, 세그먼트 탭, 좌측 "레이블+값" 카드 목록, 우측 초록 테두리 "레이블-값" 행 목록)로
   재작업. **원문 hex/CP949 패널은 완전히 제거**했다 — "구현할 것"의 원문 패널 요구사항보다 이 확정이
   우선한다. `PosTelegramDisplayFormatter.cs`(원문 포맷터)는 더 안 쓰여 삭제했다.
2. *"왼쪽=요청, 오른쪽=응답인데 kiosk 기준으로 다시 나눠야 한다"* — 좌/우 분리 기준을 "요청 전문의 모든
   필드(카드리딩만 회색)"에서 **SET 장소 기준**으로 바꿨다: 요청(좌) = kiosk 포함 필드만, 응답(우) =
   kiosk 없는 필드 중 원캡 담당 제외(단 `800000 #14`는 예외로 응답에 유지). 해당 없는 필드는 회색/공백이
   아니라 **목록에서 완전히 제거**한다(`PaymentTelegramTabViewModel.RebuildRequestRows`/
   `RebuildResponseRows`가 각각 `_kioskFieldNumbers`/`_responseFieldNumbers`로 필터링). `PosRandomValueGenerator`도
   같은 기준으로 맞춰 고쳤다(P29-4 "재확정" 절 참고) — 그렇지 않으면 화면에는 안 보여도 실제로 나가는
   바이트에는 kiosk가 안 채울 자리까지 임의값이 들어가는 모순이 생긴다.

**검증**: `dotnet build` 경고 0/오류 0. `--random-value-generator-test`로 kiosk/비kiosk 필드 개수가
스키마마다 정확히 맞는지 확인(P29-4 재검증 로그). `--payment-screen-test`로 실제 창을 띄워 스크린샷
확보 — 501008 탭이 kiosk 담당 13개 필드만 카드 목록으로 보임(로그의 "kiosk 담당 13개"와 일치), 헤더·탭·
좌우 카드 배치가 참고 이미지 톤과 가까워짐(스크린샷: `payment_screen_kiosk_split.png`).

**교차검증(숫자로 필터링 정확성 확인)**: 스키마 전체 필드 수 − kiosk 담당 수 − (응답에서 제외되는
원캡 담당 수, 800000 제외) = 응답 표에 실제로 뜨는 필드 수가 세 전문 모두 정확히 일치함을 계산으로
확인했다(501008: 56−13−0=43, 800000: 28−11−0=17[#14 예외 포함], 902614: 54−38−8=8).

**확인 못 한 것(UIPI)**: 탭 전환 클릭, "전송" 클릭 후 응답 패널이 실제로 채워지는 모습, `902614`에서
결제 알림창이 뜨는지는 관리자 권한 창이라 자동 클릭이 막혀 **실측하지 못했다.** 코드 경로(`SendAsync`→
`PosClient`, `RebuildResponseRows` 필터링)는 직접 읽어 확인했지만, 사용자가 직접 "전송" 클릭 후 응답
카드가 의도대로 채워지는지, `902614` 로그에 카드정보·PIN 원문이 안 남는지는 **아직 실측 확인이 필요하다.**

**3라운드 — 사용자가 501008 화면을 보고 다시 지적(2026-09-18, 같은 날)**: "요청의 `#5`/`#13`은 왜 있고,
`#3`/`#5`/`#6`/`#7`/`#8`/`#11`/`#12`/`#13`/`#16`은 왜 응답에서 빠졌냐, kiosk 외에 체크된 것도 있는데
확인해달라"는 지적이었다. 두 갈래로 나뉘었다:

1. **데이터 오류**: `#5`/`#13`은 501008에서 애초에 kiosk 담당이 아니었다(위 P29-4 "데이터 오류 정정"
   절 — `pos-onecap-spec-expert` 재확인, SPEC p.8 kiosk 칸 빈칸). 코드를 고쳐 501008 요청 표에서 사라졌다.
2. **로직 오류(더 큰 문제)**: 좌/우를 **배타적 분리**(kiosk 있으면 요청에만, kiosk 없으면 응답에만)로
   짰던 게 틀렸다 — kiosk와 다른 주체가 같이 체크된 필드(`#3`/`#6`/`#8`처럼 "전문을 송신하는 기관에서
   SET"하는 방향성 필드, SPEC p.6 각주)는 kiosk가 요청에 실어 보낸 값과 다른 주체가 응답에 채워 돌려준
   값이 **서로 다르므로** 요청·응답 **양쪽에 다** 나타나야 한다. `PaymentTelegramTabViewModel`에
   `HasNonKioskOwner` 판정을 추가해 응답 포함 조건을 "kiosk가 없음"에서 "kiosk 외 주체가 하나라도
   있음"으로 바꿨다(요청 포함 조건은 그대로 "kiosk가 있음" — 두 조건이 이제 겹칠 수 있다). PRD §12.5도
   같이 갱신(배타적 분리라는 서술 삭제).

재검증: `--random-value-generator-test` — 501008 kiosk 담당이 13→11개로 줄어듦(데이터 정정 반영),
800000/902614는 변경 없음. `--payment-screen-test`로 501008 요청 표에 정확히 11개(#1/#2/#3/#4/#6/#8/#9/
#11/#12/#14/#16)만 남은 것을 스크린샷으로 확인(`payment_screen_ownership_fix.png`).

**사용자가 직접 "전송" 클릭 후 실측**: 응답 표에 `#3`(0210)/`#5`/`#6`(C)/`#7`(000)/`#8`(전송일시)/`#10`/
`#11`(46)이 뜨는 것을 스크린샷으로 확인 — `#3`/`#6`/`#8`이 요청·응답 양쪽에 다 나타나는 것까지 실측
확인됐다(`HasNonKioskOwner` 로직이 실제로도 맞게 동작함).

**4라운드 — 사용자가 전송 후 화면을 보고 재지시(2026-09-18, 같은 날)**: "빈 값 행과 색 강조를 굳이
구분하지 말아달라, VAN 스텁 경고(빨간 배너)도 지워달라 — 어차피 실 VAN 붙으면 필요 없어지는 것
아니냐, 그리고 응답 패널 스크롤바가 텍스트에 너무 붙어 있다"는 지적이었다. 반영:
- 빈 값 행에 별도 표시를 추가하지 않았다(원래도 없었음 — 그대로 유지).
- VAN 스텁 빨간 경고 배너를 완전히 제거했다. 그 배너가 설명하던 "스텁이 덮어쓴 4개 필드" 초록 강조
  (`IsStubOverwritten`)도 같이 제거했다 — 배너 없이 초록색만 남으면 이유 없는 강조가 되기 때문이다.
  `IsCardReadingField`(파란 행, 원캡 카드리딩 담당) 강조는 유지했다 — 스텁과 무관하게 실 VAN이 붙어도
  계속 의미 있는 구분이라서다.
- 요청·응답 양쪽 `ScrollViewer`에 `Padding="0,0,10,0"`을 추가해 스크롤바와 텍스트 사이 여백을 뒀다.
- `PosFieldRowViewModel`/`PaymentTelegramTabViewModel`에서 `IsStubOverwritten` 프로퍼티·인자를 완전히
  제거했다(죽은 코드로 남기지 않음).

재빌드(`dotnet build` 경고 0/오류 0) 후 `--payment-screen-test`로 재확인 — 빨간 배너 사라짐, 요청 표
11개 필드 그대로(스크린샷: `payment_screen_no_banner.png`).

**5라운드 — 사용자가 이어서 지적**: "파란 행 설명도 지워줘". `#3`/`#6`/`#7`/`#8`뿐 아니라
`IsCardReadingField`(파란 행)를 설명하던 캡션(`"파란 행 = 원캡이 카드리딩으로 채운 필드..."` TextBlock)도
...
**7라운드 — 사용자가 균등폭 탭의 말줄임을 보고 지적**: "숫자는 없애고 한글 문구만 넣자, 그리고 아래
필드랑 탭 가로 위치도 맞춰줘"(정렬 자체는 6라운드에서 이미 고쳐졌고, 이번엔 재확인 겸 지적이었다).
`PaymentScreenViewModel`의 탭 라벨에서 거래 구분 코드(`501008`/`800000`/`902614`) 숫자 접두사를 빼고
한글 문구만 남겼다("국고 상세 고지내역 조회"/"카드 정보 조회"/"국고 신용카드 승인요청") — 코드값
자체는 `PaymentTelegramTabViewModel.TransactionTypeCode`로 여전히 확인 가능하다. 재빌드·재실행으로
탭 3개 전부 말줄임 없이 표시되고 정렬도 그대로 유지됨을 스크린샷으로 확인
(`payment_screen_tabs_shortened.png`).

**8라운드 — 사용자가 손그림(화살표)으로 정정**: 7라운드까지의 "정렬"을 오해했다 — 탭을 요청 필드
영역과 같은 폭으로 늘리는 게 아니라, **탭은 짧게 두고 요청 필드 쪽이 가용 폭을 더 채워야** 참고
이미지처럼 깔끔하다는 뜻이었다. 6라운드에서 도입한 `UniformGrid`(탭을 `Grid.Column="0"` 전체 폭으로
늘림)를 되돌렸다:
- `PaymentTabItemStyle`에 고정 `Width="200"` 추가, 탭 줄 `Border`를 다시 `HorizontalAlignment="Left"`로,
  `TabControl.Template`을 일반 `TabPanel`로 복원 — 탭 3개가 짧게 왼쪽에 붙는다(균등폭은 고정 Width가
  계속 보장).
- `RequestFieldCardTemplate` 카드 폭을 `256`→`205`로 줄여 한 줄에 3개가 들어가게 해서 요청 필드
  영역이 가용 폭(`Grid.Column="0"`의 `"*"` 폭, 계산상 약 692px)을 더 채우게 했다.

재빌드 경고 0/오류 0, `--payment-screen-test`로 탭이 짧아지고 요청 필드가 3열로 배치돼 탭 오른쪽
끝과 필드 영역 오른쪽 끝이 거의 일치함을 스크린샷 확인(`payment_screen_3col.png`).

**9라운드 — 사용자가 다시 지적, 여러 차례 실측 조정 끝에 최종 확정**: "탭이 이번엔 더 짧다"(8라운드가
반대 방향으로 과했다), "응답 값을 복사할 수 있게 해달라", "응답 박스 폭을 넓히고 스크롤바 간격도
띄워달라" → 잠시 440으로 넓혔다가 "이전 버전(380)이 낫다"고 되돌림 → "요청 필드가 3열이 아니다"
(205px가 실측으로는 2열까지만 들어감을 화면으로 확인) → 최종적으로:
- 탭 줄을 8라운드 이전(6라운드)으로 되돌려 다시 `UniformGrid`로 `Grid.Column="0"` 전체 폭을 채우게
  했다 — 픽셀 값을 손으로 맞추는 것보다 이 방식이 필드 카드가 몇 열이 되든 항상 탭과 폭이 맞아
  근본적으로 안정적이라고 판단했다.
- 요청 필드 카드 폭을 `205`→`160`으로 다시 줄여(실측 확인 후) 3열이 확실히 들어가게 했다.
- 응답 박스 폭은 `380`으로 유지(440에서 되돌림).
- 응답 행의 값(`ResponseFieldRowTemplate`)을 `TextBlock`→읽기전용 `TextBox`(테두리·배경 제거)로 바꿔
  선택/복사가 가능하게 했다.
- 응답 행 세로 `Padding`을 `7`→`4`로 줄여 줄 간격을 좁혔다.
- 요청·응답 `ScrollViewer` `Padding`을 `10`→`20`으로 늘려 스크롤바와 텍스트 사이 여백을 넓혔다.

매번 사용자에게 **떠 있는 결제 창을 전부 닫아달라고 요청한 뒤** 빌드 → 새 인스턴스 1개만 실행 →
스크린샷으로 재확인하는 것을 반복했다(관리자 권한 창이 여러 개 쌓이면 빌드 산출물이 잠기고, 어느
창이 최신 빌드인지 혼동되는 문제가 실제로 발생했었다). 최종 스크린샷(`payment_screen_160.png`)에서
3열 배치와 좁은 줄 간격, 사용자가 직접 전송해 응답 값(`#3`=0210/`#6`=C/`#7`=000)까지 확인됐다.

**10라운드 — 손그림으로 재지적, 최종 확정**: "탭이 요청 필드 길이와 안 맞는다(여전히 더 길다), 응답
박스를 다시 넓히고, 응답 줄 간격을 더 좁혀달라". 9라운드의 "탭을 `Grid.Column` 전체 폭에 맞춘다"는
접근 자체가 근본 원인이었다 — 요청 필드가 실제로 3열만 쓰는데 탭은 항상 컬럼 전체 폭(응답 박스
직전까지)을 채우니 열 수와 무관하게 탭이 필드보다 길어 보이는 구조적 문제였다. **탭 폭을 컬럼 전체가
아니라 필드 3열의 실제 콘텐츠 폭에 고정값으로 맞췄다**(`Width="522"` = 카드 174px×3, `HorizontalAlignment
="Left"`) — 카드 폭이 나중에 바뀌면 이 숫자도 같이 손봐야 함을 주석으로 남겼다. 응답 박스는 `380`→
`440`, 응답 행 세로 `Padding`은 `4`→`2`로 더 좁혔다(구분선 `Margin`도 맞춤).

`--payment-screen-test` 재확인 — 탭 오른쪽 끝이 요청 필드 3열 영역과 맞고, 응답 박스가 넓어지고, 줄
간격이 좁아짐을 스크린샷 확인(`payment_screen_10round.png`).

**11라운드 — 탭 글씨 잘림 + 줄 간격 추가 요청**: 522px 고정폭에서 "국고 상세 고지내역 조회" 등 긴 라벨이
말줄임됐다. 카드 폭 `160`→`180`, 연동된 탭 고정폭도 `522`→`582`(194×3)로 확대해 전부 잘리지 않고
표시되게 했다. 응답 행 세로 `Padding`은 `2`→`0`까지 줄였다(구분선 `Margin`도 `0`). 재빌드·재확인 —
탭 3개 전부 말줄임 없이 표시, 탭/필드 폭 일치 스크린샷 확인(`payment_screen_11round.png`).

### 완료 조건

- [x] 창이 열리고 3전문 전부에서 요청 필드 표가 임의값으로 채워져 보인다 — 스크린샷으로 확인(501008,
      3라운드 정정 후 11개 필드).
- [x] **(2026-09-18 재확정) 카드리딩 필드는 요청 표에 아예 나타나지 않는다**(회색 읽기전용이 아니라
      완전히 제외 — kiosk 담당이 아니므로 응답 표로 이동). 원래 조건("카드리딩 필드가 읽기전용·공백으로
      표시된다")은 이 확정으로 대체됨.
- [x] `전송` 후 응답 필드 표가 채워진다 — **사용자가 직접 전송해 스크린샷으로 실측 확인.**
- [x] ~~응답 패널에 "VAN 스텁 = 요청 에코" 경고가 보이고, 스텁이 덮어쓴 `#3`/`#6`/`#7`/`#8`이 구분된다~~
      — **(2026-09-18 4라운드) 경고 배너·스텁 강조를 전부 제거했다**(사용자 확정 — 실 VAN 붙으면
      어차피 무의미해질 표시라 처음부터 안 둔다). `#3`/`#6`/`#8`이 요청·응답 양쪽에 나타나는 것(kiosk와
      다른 주체가 같이 체크된 필드라서) 자체는 유지되고 실측 확인됨.
- [ ] `902614` 전송 시 결제 알림창이 이 창 위에 뜨고, 취소·ESC가 그대로 동작한다 — **실측 필요.**
- [x] 코드비하인드에 비즈니스 로직이 없다(창 핸들 관련 코드만) — `PaymentScreenWindow.xaml.cs`에
      DWM 타이틀바 설정만 있음을 확인, `ViewModels/Payment/*.cs`에 `System.Windows` 참조 없음(grep 확인).
- [ ] 로그에 카드정보·PIN 원문이 남지 않는다(`902614` 전송 후 로그 파일 확인) — **실측 필요**(이
      화면 코드는 `FileLogger`를 직접 호출하지 않아 기존 마스킹 정책이 그대로 적용될 것으로 판단하지만
      실측은 아니다).

**남은 실측 항목(P29-8 실기 검증으로 넘기거나, 사용자가 직접 확인)**: 전송 클릭 후 응답 패널, 902614
알림창 동작, 로그 무유출 — 셋 다 리더기/실제 클릭이 필요해 이 세션의 UIPI 제약으로 못 끝냈다.

## P29-7. 홈 화면 연결 (PRD §12.1, §12.4)

### 구현할 것

- `ViewModels/HomeViewModel.cs`의 `OpenTrans()`가 `NotImplementedCardRequested`를 올리는 대신 결제 화면을
  열게 한다. 창 생성은 View 계층 책임이므로 **ViewModel이 `Window`를 직접 new 하지 않는다** — 기존
  설정 화면(`ReaderSetupWindow`/`ShopSetupWindow`)이 홈에서 열리는 방식을 그대로 따른다.
- **`SetupScreenGate`에 등록하지 않는다**(PRD §12.4). 이 판단의 근거(단일 워커 Queue가 순차 처리하므로
  리더기·VAN 동시 접근이 없다)를 코드 주석에도 남긴다 — 나중에 "왜 설정 화면과 다르게 처리했나"가
  반드시 의문이 된다.
- 중복 실행 방지: 이미 열려 있으면 새 창을 만들지 않고 기존 창을 활성화한다(설정 화면과 같은 처리).
- 카드 설명 문구는 실사용 결제 화면과 이미 맞으므로 **문장 자체는 유지**하고, **원본 오탈자
  "결제을"만 "결제를"로 고친다**(2026-09-18 사용자 확정 — 사용자에게 보이는 문구라 원본 재현 원칙보다
  가독성을 택했다). `docs/home_reader_setup/PRD_WPF.md`의 홈 화면 카드 텍스트 표(line 175)도 같이
  갱신하고, **원본 MFC는 "결제을"이라는 사실을 그 표에 각주로 남긴다** — 나중에 원본과 대조할 때
  불일치로 오판하지 않도록.

### 완료 조건

- [x] 홈 "결제" 카드 설명이 "결제 화면을 실행하고\n결제를 진행합니다"로 표시되고 `PRD_WPF.md`도 갱신됐다.
  실측(스크린샷) 확인, 2026-09-21.
- [x] 홈 "결제" 카드 클릭 → 결제 화면이 열린다(MessageBox가 더는 뜨지 않는다). 실측(사용자 직접 클릭)
  확인, 2026-09-21 — 관리자 권한 창이라 `mcp__windows__*` 자동화(list_windows/snapshot/click 전부)가
  UIPI로 막혀 클릭은 사용자가 직접 수행.
- [x] 두 번 클릭해도 창이 하나만 존재한다. 실측(사용자 직접 확인) 완료, 2026-09-21.
- [x] 결제 화면이 열린 상태에서 소켓으로 들어온 전문이 `E03`으로 거절되지 않는다. **코드 추적만**
  — `PaymentScreenWindow.xaml.cs`에 `SetupScreenGate.Register()`/`Unregister()` 호출이 없음을 grep으로
  확인(`IsSetupScreenOpen` 카운터가 이 창의 개폐에 영향받지 않음). 실기 소켓 전문 왕복 실측은 P29-8로 넘김.
- [x] 리더기/가맹점 설정 화면의 기존 `E03` 동작은 그대로다(회귀 없음). 코드 추적 — 이번 변경이
  `SetupScreenGate.cs`/`ReaderSetupWindow`/`ShopSetupWindow`를 건드리지 않았음을 diff로 확인.

## 체크포인트 2 ★ — Opus 검증 리뷰 (P29-7 직후)

**이 Phase에서 가장 위험한 지점.** P29-6·P29-7이 처음으로 기존 결제 경로(홈 화면 진입점,
`SetupScreenGate`)를 실제로 배선한다 — 여기서 잘못되면 새 화면이 아니라 **기존 POS 경로가 깨진다.**
리뷰 대상:

- `SetupScreenGate`에 결제 화면을 등록하지 않은 것이 실제로 안전한가 — 단일 워커 `TransactionQueue`가
  정말 리더기·VAN 동시 접근을 막는지 코드로 재확인(가정이 아니라 확인)
- `HomeViewModel.OpenTrans()` 교체가 기존 설정 화면 진입 패턴(창 생성 책임이 View 계층에 있는 것,
  중복 실행 방지)과 정확히 같은 방식인가 — 다르게 만들었다면 그 이유가 타당한가
- 결제 화면이 열린 상태에서 실제 POS 소켓 요청이 `E03`으로 거절되지 않는가(완료 조건 그대로 재현),
  반대로 **기존 설정 화면들의 `E03` 동작에는 회귀가 없는가**
- P29-6의 VAN 스텁 경고 표시가 실제로 눈에 띄고, 스텁이 덮어쓴 4개 필드(`#3`/`#6`/`#7`/`#8`)와 나머지
  에코 필드가 화면에서 구분되는가
- 로그 마스킹이 화면 표시에는 적용되지 않되 **실제 로그 파일에는 여전히 적용되는가**(§4.13 — 카드정보·
  PIN이 로그에 새지 않는지 P29-6 완료 조건과 별개로 재확인)

### 검증 결과

### 검증 결과 (2026-09-21, `checkpoint-reviewer` 독립 세션, Opus)

**리뷰 대상 5개 항목** — 전부 통과:
1. `SetupScreenGate` 열외 안전성 — `TransactionQueue`가 전용 워커 스레드 1개(`BlockingCollection`)로
   FIFO 직렬화하고, `App.PaymentQueue` 단일 인스턴스를 `PosSocketServer`가 공유함을 코드로 확인. 결제
   화면은 오케스트레이터를 직접 부르지 않고 `PosClient` → 소켓만 사용해 구조적으로 리더기/VAN 동시
   접근이 발생하지 않는다.
2. `OpenTrans()` 교체 패턴 — 기존 두 설정 카드와 동일한 이벤트 기반 구조(View가 창 생성 책임). `Show()`
   (비모달) + `SetupScreenGate` 열외는 실수가 아니라 PRD §12.4 근거가 있는 의도된 설계 차이임을 확인.
3. `E03` 비거절/기존 설정 화면 회귀 없음 — `PaymentScreenWindow`에 `SetupScreenGate.Register/Unregister`
   호출이 없고, `ReaderSetupWindow`/`ShopSetupWindow`/`SetupScreenGate.cs`를 이번 변경이 건드리지 않았음을
   diff로 확인(코드 추적, 회귀 하네스 실측은 UAC 승격 거부로 이 세션에서 불가 — 아래 "실측 미확인" 참고).
4. VAN 스텁 경고 배너 제거 — P29-6 4·5라운드 기록과 PRD §12.5 갱신에 근거가 있는 확정된 설계 결정.
5. 로그 마스킹 우회 없음 — `ViewModels/Payment/` 전체에 로깅 호출 0건(grep 확인), 기존 `PosSocketServer`/
   `VanService`의 리댁션 경계만 통과한다.

**발견된 결함(전부 수정 완료)**:
- **M-1**: 결제 화면을 최소화한 상태에서 홈 "결제" 카드를 다시 누르면 `Activate()`만으로는 최소화가
  풀리지 않아 무반응으로 보임 → `HomeWindow.OpenPaymentScreen()`에 `Show(); WindowState = Normal;
  Activate();` 순서 적용(`RestoreFromTray()`와 동일 패턴). 수정 완료.
- **M-2**: 탭별 `IsSending`이 독립이라 한 탭이 전송 중(예: 902614, 카드 대기 최대 120초)이어도 다른
  탭으로 전환해 그 탭의 전송을 누를 수 있었다 — 응답 타임아웃에 먼저 걸린 요청이 화면엔 "실패"로
  보여도 서버에서는 정상 처리돼 사용자가 오인 재전송할 위험. **사용자 확정(2026-09-21, "통신중일때는
  다른 걸 못하게 하는게 맞아")**: 탭 중 하나라도 전송 중이면 `TelegramTabControl` 자체를 비활성화해
  탭 전환을 막는다(`PaymentScreenViewModel.IsAnyTabSending` 신설). 수정 완료.
- **L-1**: `new PaymentScreenWindow { Owner = this }`(객체 이니셜라이저)는 생성자 실행 후 Owner를
  대입하므로, 생성자 본문의 `Owner == null` 판정이 홈 경로에서도 항상 참이어서 `CenterOwner`가 한 번도
  적용되지 못했다 → Owner를 생성자 매개변수로 받는 `PaymentScreenWindow(Window? owner)`로 변경, 판정
  시점에 Owner가 이미 확정되도록 수정. 수정 완료.
- **L-2**: "아직 홈 화면과 배선되지 않는다" stale 주석 4곳(`PaymentScreenWindow.xaml(.cs)`,
  `PaymentScreenViewModel.cs`, `App.xaml.cs`) + `HomeWindow.OnNotImplementedCardRequested` 요약이 여전히
  "결제/전표 설정 카드는 범위 밖"으로 남아 있던 것. 전부 정리 완료.
- **L-3**: `PRD.md` §12.1이 "결제을" 문구를 "그대로 두어도 맞다"고 서술해 P29-7의 "결제를" 수정과
  어긋났던 것, `PRD_WPF.md` §3.7이 "가맹점 설정/결제/전표 설정 카드는 범위 외"로 남아 있던 것. 둘 다
  갱신 완료(전표 설정만 범위 외로 남음을 명시).
- **L-4**: `RefreshStatusBadge()`의 분기 순서(`!HasResponse`가 `IsStatusError`보다 먼저)가 전송 계층
  실패(연결 실패/타임아웃)나 필드 길이 초과 시에도 배지를 "미전송"으로 표시해 좌측의 빨간 "전송 실패"
  메시지와 모순됐다 → `IsStatusError`를 `!HasResponse`보다 먼저 검사하도록 순서 수정. 수정 완료.
- **L-5**: 빌드 임시 파일(`*_wpftmp.csproj`)이 워킹 트리에 untracked로 남아 있던 것 → 삭제 + `.gitignore`에
  `*_wpftmp.csproj` 패턴 추가. 수정 완료.

**실측 미확인(추측하지 않고 명시)**: `--payment-flow-test`/`--pos-client-test` 회귀 하네스는 `app.manifest`의
`requireAdministrator` 때문에 비관리자 세션에서 UAC 승격이 거부되어 이 체크포인트 세션에서 실행하지
못했다. 코드 추적(변경 파일이 게이트 관련 코드를 건드리지 않음)으로만 회귀 없음을 확인했고, **P29-8
실기 검증에서 관리자 권한 세션으로 반드시 재확인**한다.

수정 후 `dotnet build` 재확인 — 경고 0 / 오류 0.

**판정: 통과 (M-1·M-2·L-1~L-5 전부 수정 완료, 재검증 불필요 — 수정이 기계적이고 리뷰가 지정한 5개
항목 자체는 최초 리뷰에서 이미 통과였음).** P29-8(실기 검증)에서 회귀 하네스 실측 확인만 이어서 진행.

## P29-8. 검증 (ROADMAP Phase 29 완료 기준)

### 구현할 것

- 실 리더기로 `501008`(리더기 불필요) / `800000`(카드 1회) / `902614`(카드 + PIN) 각 1회 왕복.
- **원캡 담당 필드가 공백에서 실제 값으로 바뀐 것**을 화면과 로그 양쪽에서 대조한다. 로그는
  `KFTCTaxCAP{yyMMdd}.log`이고 상관키는 `#9`다.
- 스크린샷 확보(`mcp__windows__windows_screenshot`). **주의**: 앱이 관리자 권한으로 떠 있으면 UIPI 때문에
  GUI 자동화가 그 창에 입력할 수 없다 — 그 경우 사용자에게 직접 조작을 요청한다.
- 회귀: `--pos-client-test`, `--payment-flow-test`가 그대로 통과.
- `dotnet build` 경고 0 / 오류 0.

### 완료 조건

- [x] 3전문 각 1회 왕복 성공(실기, 2026-09-21). 상관키: 501008/800000 = `ptbOOWI9MZXz`,
      902614 = `05Sq9g8y55Rt`. 셋 다 `거래 확정 [000]`.
- [x] `800000 #14` BIN과 `902614` 8개 필드가 실제 카드 데이터로 채워진 것이 확인된다 — 카드리딩 전
      POS 요청 원문은 해당 구간이 공백이고, 카드리딩 후 VAN 중계 원문에서 `800000 #14`(BIN
      `52728983`)와 `902614 #43~#46,#48,#50,#51,#53`(8개, `"승인요청 필드 8종 채움 완료"` 로그로
      확정)가 실값으로 채워진 것을 로그에서 바이트 단위로 대조.
- [x] 응답·로그에 카드정보·PIN 잔존이 없다(§4.13 규칙이 그대로 지켜진다) — POS로 보내는 응답 원문은
      카드번호가 마스킹(`F1NseS***719s`)돼 있고 PIN(#51)/EMV(#53) 구간은 공백. PIN 입력 로그는
      `"PIN 4자리 입력 완료 — 통신중으로 진행(값은 로그에 남기지 않음)"`만 남고 값 자체는 어떤
      로그에도 없음.
- [x] 회귀 하네스 2종 통과 — `--pos-client-test` 시나리오 1~9 전부 완료, `--payment-flow-test` **통과
      171건 / 실패 0건**(관리자 권한 세션에서 재실행, CP2가 미확인으로 남겼던 항목 해소).
- [x] `dotnet build` 경고 0 / 오류 0(2026-09-21 최종 확인).
- [x] 스크린샷 확보 — `docs/payment_relay/images/p29-8_realtest_result.png`(902614 탭, 응답 코드
      000·전송 완료 화면).
...
1. P29-1~P29-8의 모든 완료 조건 통과. **(2026-09-21 전부 통과 — Phase 29 완료)**
2. 홈 "결제" 카드에서 결제 화면이 열리고 3전문이 각각 왕복한다.
3. **원캡 담당 필드가 실제 카드 데이터로 덮인 것이 화면에서 확인된다.**
4. 기존 POS 경로(키오스크 → 소켓)의 동작이 바뀌지 않는다(회귀 없음).
5. 아래 "열린 항목"이 문서에 정확히 기록된다.

**전문 간 연쇄는 이 Phase의 완료 기준이 아니다.**

## 열린 항목 (이 Phase로 닫히지 않는 것)

- **전문 간 필드 연쇄 매핑** — `spec_open_questions.md` Q1~Q11 발주처 답변 대기. 해소 후 PRD §3.3.2에
  매핑을 옮기고 별도 Phase로 착수한다(번호는 그때 배정).
- ~~**VAN Stub 응답 내용**~~ — **2026-09-18 해소.** 스텁은 요청을 clone해 `#3`/`#6`/`#7`/`#8`만 덮어쓰는
  **에코**임을 확인했다(공백이 아니다). 스텁을 수정하지 않고 **화면에 경고를 표시**하는 쪽으로 확정
  (P29-6). 실 VAN이 붙으면 이 경고를 제거한다.
- **실 VAN 서버** — Phase 20/21에서 이어지는 서버 대기 항목(`nRet==0` relay 경로 미검증).
- **소켓 경유 vs 오케스트레이터 직통** — 원캡 단독 화면이므로 장래에 직통으로 바꿀 선택지가 있다.
  이번에는 검증 범위를 넓게 두려고 소켓 경유를 택했다(PRD §12.4).
- **`#4` 채번 / `#53` 응답 방향** — Phase 26에서 이어지는 발주처 대기 항목.

## 착수 순서 요약

P29-1(문서) → P29-2(응답코드 대조) → P29-3(`031`) → **P29-4(임의값 생성기)** → P29-5(소켓 클라이언트)
→ **체크포인트 1(Opus 검증)** → P29-6(화면) → P29-7(홈 연결) → **체크포인트 2(Opus 검증)** → P29-8(검증).

**P29-4를 P29-6보다 먼저** 하는 것이 이 순서의 핵심이다 — 임의값이 스키마를 통과해 실제로 왕복
가능한지가 먼저 확정돼야, 화면 작업 중에 "값이 잘못됐는지 화면이 잘못됐는지" 두 가지를 동시에 의심하지
않는다. P29-2/P29-3은 화면과 독립적이라 앞에 두어 문서 작업의 연장선에서 끝낸다.

---

# Phase 30 실행계획서 — 전문 간 필드 연쇄

> **근거**: `PRD.md` §13(전문 간 필드 연쇄 요구사항), §3.3.2(값 출처 매핑표 정본), §13.2(연쇄는 화면
> 책임), §13.7(값 변환 규칙) · `ROADMAP.md` Phase 30 · `spec_open_questions.md` §4(근거 구분 — SPEC
> 명문 vs 사용자 확정)

**이 Phase가 끝나면**: 결제 화면에서 `501008`을 보내면 그 응답값이 `800000`·`902614` 탭의 요청 필드에
자동으로 채워지고, `800000`을 보내면 그 응답값이 `902614`에 채워진다. **앞 전문 응답의 납세의무자번호·
징수기관명·본세 등이 `902614` 요청 원문에 실제로 실려 나간 것이 로그에서 확인된다.**

## 이 Phase의 성격

Phase 29가 의도적으로 범위 밖에 뒀던 연쇄를 얹는다. Phase 29 착수 당시(2026-09-18)에는 연쇄 근거가
구간마다 갈려 "우리 추정이 정본으로 굳는 것"을 피하려고 미뤘는데, 2026-09-21~22에 **필드별 값 출처가
전부 확정**되면서(PRD §3.3.2) 그 이유가 사라졌다.

**단, SPEC 원문 명문 근거는 여전히 대부분 없다.** `spec_open_questions.md` §4의 "SPEC 명문 = 없음" 행
(Q1~Q3, Q5~Q8, Q10)이 그것이다 — 값은 확정됐지만 SPEC 문장으로는 안 적혀 있다. SPEC 자체를 우리 측이
작성·관리하므로 구현을 막지는 않으나, **매핑을 코드에 흩뿌리지 말고 한 곳에 모아 두는 것**이 이 Phase
설계의 핵심 제약이다(P30-2) — 다음 SPEC 개정에서 매핑이 또 바뀔 수 있다.

## 이 Phase의 절대 제약 — 원캡 중계 경로를 건드리지 않는다

연쇄는 **결제 화면(키오스크 역할 대행)의 책임**이다(PRD §13.2). 실제 POS가 보내는 요청은 **키오스크가
이미 연쇄를 끝내고** 보낸 것이므로, 원캡이 요청 필드를 덮어쓰면:

1. §4.10 relay 원칙이 깨진다(원캡은 요청을 해석·수정하지 않는다).
2. **실제 POS 거래의 동작이 바뀐다** — 이 Phase가 건드려선 안 되는 영역이다.

**따라서 다음 파일이 이 Phase의 `git diff`에 나타나면 설계를 잘못 잡은 것이다**(Phase 26의 같은 방식
점검을 따른다 — 완료 조건에 `git diff` 확인 포함):

- `Services/Payment/PaymentOrchestrator.cs`
- `Services/Payment/TransactionQueue.cs`
- `Services/Pos/PosSocketServer.cs`

## 착수 전 전제 (2026-09-22 코드 확인)

1. 3전문 스키마는 이미 있고 20260922판에서도 필드 구성·POSITION·총 길이가 바뀌지 않았다 —
   `Protocol/Pos/Schemas/`를 손대지 않는다. 단 `CardInfoInquirySchema`의 `#11`/`#12` owner는
   2026-09-22에 `Kiosk`→`None`으로 이미 정정됐다(SPEC 개정 반영, 이 Phase 착수 전 완료).
2. `PosTelegramSchema`가 POSITION 연속성·총 길이를 자기 검증하고 `FieldsOwnedByOneCap()`을 제공한다.
3. `PosField.Pad()`가 패딩을 담당한다(`N`=우측정렬 `'0'`, 그 외=좌측정렬 space, 초과 시 예외) —
   **길이 확장(N12→N15)에 별도 변환 코드를 두지 않는 근거**다(PRD §13.7).
4. `PosRandomValueGenerator`(P29-4)가 스키마 메타만 보고 임의값을 만든다 — P30-1의 스텁 확장과
   P30-5의 재생성이 이것을 재사용한다.
5. `Services/Pos/PosClient.cs`(P29-5)가 소켓 왕복을 담당한다 — 이 Phase에서 전송 경로는 바꾸지 않는다.
6. 화면은 `Views/PaymentScreenWindow.xaml` + `ViewModels/Payment/PaymentScreenViewModel.cs` +
   `PaymentTelegramTabViewModel.cs` + `PosFieldRowViewModel.cs` 구조다(P29-6). 연쇄 상태는 여기에 얹는다.
7. `Services/Van/StubVanRelayService.cs`는 요청을 clone해 `#3`/`#6`/`#7`/`#8` 네 개만 덮어쓴다 —
   **P30-1이 이걸 확장한다.**
8. `PaymentScreenWindow`는 `HomeWindow.OpenPaymentScreen()`이 인스턴스를 캐시한다(P29-7) —
   **응답 캐시 생명주기(창 생존 기간)가 이 캐시와 일치**한다(PRD §13.6).

## 이 Phase에서 손대지 않는 것 (범위 밖)

- **원캡 중계 경로** — 위 "절대 제약" 3개 파일.
- **3전문 스키마** — 필드 구성·POSITION·길이 전부 불변.
- **실 VAN 연동** — `StubVanRelayService` 유지(PRD §13.8의 확장도 스텁 안에서만).
- **할부 개월 선택 UI** — `#34`는 일시불 `"00"` 고정(PRD §13.9).
- **`#14` 전자납부번호 / `#35` 연락 전화번호 실입력** — 임의값 유지(PRD §13.9).
- **`999999`(거래 상태 조회)** — 연쇄와 무관.
- **전송 경로** — 소켓 경유 그대로(PRD §12.4).

---

## P30-1. 스텁 VAN 응답 확장 (PRD §13.8)

### 왜 이게 첫 Task인가

**이게 없으면 연쇄를 검증할 수 없다.** 스텁은 요청을 clone해 `#3`/`#6`/`#7`/`#8`만 덮어쓰는데,
`501008` 응답의 업무부(`#17` 납세의무자번호, `#19` 징수기관명, `#30` 본세 등)는 전부 **디지털예산
담당**이라 화면이 채우지 않아 공백으로 나간다 → 응답도 공백 → 연쇄해도 `902614`의 해당 필드가 공백이다.

2026-09-21 P29-8 실기 로그에서 실제로 확인된 사실이다:

```
응답 송신 원문=0706IXg2930210501008   C000260921133745ptbOOWI9MZXz            814008680  Kc1wOjcl...
                                                                        (이후 업무부 전부 공백)
```

### 구현할 것

- `StubVanRelayService`가 응답 전문을 만들 때, **kiosk가 SET 장소에 없는 응답 필드**를
  `PosRandomValueGenerator`로 채워 반환한다.
- **원캡 담당 필드는 덮지 않는다** — `800000 #14`(BIN), `902614`의 원캡 8개는 원캡이 카드리딩으로 채워
  보낸 **실제 값**이다. 스텁이 덮으면 카드리딩 검증이 무의미해진다(P29-8이 확인한 것을 무너뜨린다).
- 기존 `#3`(→`0210`)·`#6`(→`C`)·`#7`(→`000`)·`#8`(응답 시각) 덮어쓰기는 그대로 유지한다.
- **개발용 장치임을 주석으로 명시**한다 — 실 VAN 전환 시 `App.xaml.cs` 한 줄 교체(§10)로 함께 사라진다.
- `902614` 응답에도 같은 규칙을 적용할지: `902614`는 인터넷지로 담당 응답 필드(`#40` 기 납부 이용 시스템,
  `#52` 선불카드 잔액 등)가 있으므로 같은 규칙으로 채운다. 단 `#53 EMV DATA`는 `I | C` 공유 필드라
  **원캡이 채운 자리를 덮지 않도록 주의**한다(§4.13의 삭제 대상과도 얽힌다).

### 완료 조건

- [x] `501008` 응답에 `#17`/`#19`/`#30`/`#31`/`#32`/`#44`/`#55` 등 디지털예산 담당 필드가 공백이 아닌
      값으로 채워져 돌아온다(로그 원문으로 확인) — `StubVanRelayService.BuildFakeSuccess`가
      `cloned.Schema.Fields`를 순회하며 kiosk 소유도 원캡 소유도 아닌 필드를 `PosRandomValueGenerator`로
      채우도록 확장(2026-09-23, `csharp-wpf-developer` 구현).
- [x] `800000 #14`(BIN)가 **원캡이 카드리딩으로 채운 값 그대로** 응답에 남아 있다(스텁이 덮지 않았다) —
      `HasFlag(PosFieldOwner.OneCap)` 필드는 루프에서 스킵되도록 구현·코드 리뷰로 확인.
- [x] `902614`의 원캡 담당 8개 필드도 스텁이 덮지 않는다 — 8개 전부 `PosFieldOwner.OneCap` 플래그를
      가지므로 동일 조건으로 스킵됨(`#53 EMV DATA`의 `I | C` 조합 포함, 확인 완료).
- [x] 응답 본문 길이가 스키마 총 길이(706/500/1500)와 정확히 일치한다(`0x00` 없음) — `dotnet build`
      경고 0/오류 0, `PosField.Pad`가 예외 없이 통과.
- [x] 기존 회귀(`--pos-client-test`, `--payment-flow-test`)가 그대로 통과한다 — 스텁 응답이 바뀌면
      기존 하네스의 기대값이 깨질 수 있으므로 **이 확인이 특히 중요하다.** `app.manifest`를 일시
      `asInvoker`로 낮춰 실행(2026-09-23) → `--pos-client-test` 9개 시나리오 전부 정상(ERROR 0건,
      `KFTCTaxCAP260923.log` 09:42:32~09:43:03), `--payment-flow-test` 171건 통과/0 실패(09:44:29) →
      확인 후 `app.manifest`를 `requireAdministrator`로 원복(`git diff` 클린 확인).

---

## P30-2. 연쇄 매핑 정의 (Protocol 계층)

### 구현할 것

PRD §3.3.2의 매핑표를 **선언적 테이블 하나**로 옮긴다(`Protocol/Pos/` 아래). 한 항목은 다음을 담는다:

- 출처: 전문 종별(`501008`/`800000`) + 응답 필드번호 (또는 "고정값" / "합산")
- 대상: 전문 종별(`800000`/`902614`) + 요청 필드번호
- 변환 종류: 그대로 / 절삭 / 합산 / 고정값

담아야 할 항목(PRD §3.3.2가 정본 — 여기 숫자를 옮길 때 **반드시 §3.3.2를 직접 보고** 옮긴다):

- `501008` 응답 → `902614` 요청: `#17`→`#14`, `#14`→`#15`, `#15`→`#16`, `#20`→`#18`, `#22`→`#19`,
  `#19`→`#20`(절삭), `#21`→`#21`(절삭), `#23`→`#22`, `#24`→`#23`, `#30`→`#24`, `#32`→`#25`,
  `#31`→`#26`, `#44`→`#30`, `#26`→`#31`, `#17`→`#36`, `#18`→`#37`(절삭)
- `501008` 응답 → `800000` 요청: `#30`+`#31`+`#32`→`#15`(합산), `#55`→`#16`
- `800000` 응답 → `902614` 요청: `#24`→`#28`, `#17`→`#33`
- 합산: `902614 #27` = `501008 #30`+`#31`+`#32` / `902614 #29` = `902614 #27`+`#28`
- 고정값: `902614 #34` = `"00"`

### 주의

- **화면·ViewModel에 필드번호를 흩뿌리지 않는다.** SPEC이 또 바뀌면 이 테이블 한 곳만 고쳐야 한다
  (2026-09-18 → 09-21 → 09-22로 세 번 바뀐 전례가 있다).
- `#29`는 **`#27`과 `#28`이 먼저 정해진 뒤** 계산된다 — 매핑 적용 순서에 의존성이 있음을 테이블에
  명시하거나, 적용을 2단계(단순 매핑 → 파생 계산)로 나눈다.
- `#38`은 **대상에 넣지 않는다** — 인터넷지로 담당이라 kiosk가 채우지 않는다(PRD §3.3.1-c).
- `800000 #11`/`#12`도 **넣지 않는다** — 2026-09-22 SPEC 개정으로 공백 처리 확정(Q9).

### 완료 조건

- [x] 매핑 테이블이 한 파일에 모여 있고, `ViewModels/`·`Views/` 어디에도 연쇄용 필드번호 리터럴이
      없다(grep으로 점검) — `Protocol/Pos/TelegramFieldChainMap.cs` 신설(2026-09-23). P30-4 이전
      단계라 `ViewModels/`·`Views/`에는 애초에 연쇄용 필드번호가 아직 없음(grep 결과 0건) — P30-4에서
      이 파일 밖에 필드번호 리터럴을 추가하지 않는지 그때 다시 확인한다.
- [x] 테이블 항목 수와 PRD §3.3.2 표의 행 수가 일치한다(누락 없음) — PRD 표의 모든 필드를 낱개로 펼치면
      23건(`501008`→`902614` 16건 + 합산 1건(`#27`) + `800000`→`902614` 2건 + `902614` 내부 파생
      합산 1건(`#29`) + 고정값 1건(`#34`) + `501008`→`800000` 2건), `Entries` 배열도 23건으로 일치.
      `#38`/`#42`/`800000 #11`·`#12`는 PRD·코드 양쪽에서 의도적 제외로 명시.
- [x] 각 항목에 근거 주석(SPEC 명문인지 사용자 확정인지)이 붙어 있다 — `ChainEntry.Evidence`마다
      `spec_open_questions.md` §4 Q번호 또는 "이름 일치(SPEC 명문 없음)"를 명시. SPEC 명문 근거가
      있는 항목은 `#29`(Q4, 2026-09-22 재배포본에서 p.16 자기참조 오탈자가 정정돼 근거 확보) 1건뿐이고
      나머지 22건은 전부 사용자 확정/이름 일치만 근거임을 확인했다.
- `dotnet build` 경고 0 / 오류 0 확인(2026-09-23).
...
PRD §13.7의 4가지 변환. 핵심은 **반글자 방지 절삭**이다.

- **절삭**: CP949에서 한글은 1자당 2바이트다. 바이트 한도까지 자를 때 마지막 글자가 반쪽이면 **그 글자를
  통째로 버린다**(남는 1바이트는 `PosField.Pad()`의 공백 패딩이 채운다). 대상: `501008 #19`→`902614 #20`
  (AHN40→20), `#21`→`#21`(AHN40→20), `#18`→`#37`(AHN40→AHNS10).
- **길이 확장**: `800000 #24`(N12)→`902614 #28`(N15)는 **변환 코드를 두지 않는다** — N형이라
  `PosField.Pad()`가 우측정렬 `0` 패딩으로 처리한다(§10.1).
- **표현 변환**: 자릿수가 같으므로 값을 그대로 옮긴다(`N3`→`AN3` 등).
- **합산**: 정수합. 자리수를 넘으면 `PosField.Pad()`가 예외를 던져 조용히 잘리지 않는다.

### 완료 조건 (단위 검증)

- [x] 절삭 경계 케이스가 전부 통과한다:
      한글만 20바이트 초과 / 한글+ASCII 혼합에서 경계가 한글 중간 / 정확히 한도에 맞는 경우 /
      한도를 1바이트만 넘는 경우 / 빈 값 / 한도보다 짧은 값(절삭 없음) — `TelegramFieldChainConverter.cs`
      신설(2026-09-23), 6케이스 전부 통과(`KFTCTaxCAP260923.log` 10:26:27).
- [x] 절삭 결과를 CP949로 인코딩했을 때 **항상 한도 이하**이고, 디코딩하면 깨진 글자가 없다 — 매 케이스
      확인 완료(대체문자 없음).
- [x] 합산 결과가 `PosField.Pad()`를 통과하고, 자리수 초과 시 예외가 난다(조용히 잘리지 않는다) —
      정상 범위(15자리) 통과, 오버플로(16자리) `PosProtocolException` 실제 발생 확인.
- [x] 진단 하네스 인자로 이 검증을 재실행할 수 있다(기존 `--random-value-generator-test` 방식을 따른다) —
      `--field-chain-converter-test` 신설(`TelegramFieldChainConverterSelfTest.cs`, `App.xaml.cs` 배선).
      실행에 `app.manifest`를 일시 `asInvoker`로 낮춰야 했다(P30-1과 동일 UAC 제약) — 검증 후
      `requireAdministrator`로 원복 확인(`git diff` 클린).
- `dotnet build` 경고 0 / 오류 0 확인(2026-09-23).
...
- 전송 성공 시 응답 전문을 **화면 창 생존 기간 동안** 보관한다(PRD §13.6). 창을 닫으면 폐기,
  **§7 SQLite에 저장하지 않는다.**
- 보관한 응답에 P30-2의 매핑 + P30-3의 변환을 적용해 **뒤 전문 탭의 요청 필드를 갱신**한다.
  - `501008` 성공 → `800000`·`902614` 탭 갱신
  - `800000` 성공 → `902614` 탭 갱신
- 선행 응답이 없으면 **임의값을 그대로 둔다**(PRD §13.4 — 순서를 강제하지 않는다).
- 갱신된 필드는 **편집 가능**하게 유지한다(읽기전용으로 잠그지 않는다, PRD §13.3).
- 필드별로 **"연쇄로 채워진 값인지"** 구분하는 상태를 둔다 — P30-5(재생성 제외)와 P30-6(색 구분)이
  이 상태를 쓴다.

### 주의

- **`PaymentOrchestrator`·`PosSocketServer`를 손대지 않는다**(이 Phase 절대 제약).
- 한 탭이 전송 중일 때 다른 탭 전송을 막는 기존 게이트(P29 CP2의 M-2 수정,
  `IsSendBlocked`/`IsBlockedByOtherTab`)를 깨지 않는다.
- 사용자가 값을 직접 고친 뒤 앞 전문을 재전송하면 어떻게 되는가 — **덮어쓴다**(연쇄가 최신 응답을
  반영하는 것이 이 기능의 목적). 다만 이 동작을 구현 주석에 명시해 나중에 "왜 내가 고친 값이
  사라지냐"는 오해가 없게 한다.

### 완료 조건

- [x] `501008` 전송 성공 직후 `902614` 탭의 `#14`/`#20`/`#24` 등이 응답값으로 바뀐다(화면 확인) —
      `--payment-screen-test`로 확인. `#14`="haEkbkjJSlhTc"(501008 `#17`과 정확히 일치), `#20`(절삭,
      한글 반글자 경계 정상), `#24`="000000065505510"(501008 `#30`과 정확히 일치), `#34`="00"(Regenerate
      직후 고정값 적용 확인), `800000 #15`도 같은 합산값으로 동시에 갱신(한 응답이 여러 대상 탭에
      팬아웃되는 것 확인). 검증 중 실제 버그 1건 발견·수정(`SendAsync`에서 `HasResponse=true`가
      `_lastResponseTelegram` 대입보다 먼저 실행돼 연쇄가 조용히 스킵되던 순서 오류 — 대입 순서 수정).
- [ ] `800000` 전송 성공 직후 `902614` 탭의 `#28`/`#33`이 바뀌고, `#29`가 `#27`+`#28`로 재계산된다 —
      배선 로직은 구현·코드 리뷰 완료. `800000`은 원캡 담당(BIN) 필드가 있어 실기(카드 리더) 없이는
      끝까지 검증 불가(E05로 실패, 예상된 동작) — **P30-7(실기 검증)에서 최종 확인**.
- [x] `501008`을 보내지 않고 `902614`를 먼저 보내도 임의값으로 정상 왕복한다(PRD §13.4) — 코드 리뷰로
      확인: `Regenerate()`의 `#27`/`#28` 축소 난수 오버라이드가 전송 순서와 무관하게 항상 적용되므로
      구조적으로 보장됨(실기 카드/PIN 필요해 라이브 재검증은 P30-7로 미룸).
- [x] 창을 닫고 다시 열면 캐시가 비어 있다(처음부터 전송해야 한다) — `_lastResponseTelegram`이 인스턴스
      필드라 `PaymentScreenViewModel`/`PaymentTelegramTabViewModel`이 새로 생성될 때마다 구조적으로
      비어 있음.
- [x] `git diff`에 `PaymentOrchestrator.cs`·`TransactionQueue.cs`·`PosSocketServer.cs`가 없다 — 확인.
      `TelegramFieldChainMap.cs`/`TelegramFieldChainConverter.cs`도 미변경.
- `dotnet build` 경고 0/오류 0, 기존 회귀 4종(`--pos-client-test`/`--payment-flow-test`(171/171)/
  `--field-chain-converter-test`/`--random-value-generator-test`) 전부 통과 확인(2026-09-23).

**추가로 발견된 열린 항목(코드는 그대로 둠, 기록만)**: 현재 연쇄 적용은 응답 코드(`#7`)를 보지 않고
프로토콜상 응답을 받으면(파싱 성공이면) 무조건 적용한다 — 업무적으로 실패한 응답(예: 데이터 없음)의
빈 값/오류값도 그대로 연쇄될 수 있다는 뜻이다. 지금 스텁(`StubVanRelayService`)은 항상 성공(`#7`="000")
만 돌려주므로 이 경로가 이번 Phase에서 실제로 발현되지 않는다 — 실 VAN 연동 시점에 응답 코드를 보고
연쇄 여부를 판단할지 다시 결정한다(§10, 아래 "열린 항목"에도 추가).
...
"임의값 재생성" 버튼이 **연쇄로 채워진 필드를 건드리지 않게** 한다(PRD §13.5). 재생성 대상은 kiosk가
자체 생성하는 필드뿐이다 — `#32` 납부일자, `#35` 연락전화번호, `#39`, `#41`, `#42`, `#49`, `#14`
전자납부번호 등(PRD §2.3 참고).

### 주의

근거를 잊지 않도록 주석에 남긴다: **재생성이 연쇄 값을 덮으면 연쇄 결과를 화면에서 확인할 수 없어
기능 자체가 검증 불가가 된다.**

### 완료 조건

- [x] `501008` 전송 후 `902614` 탭에서 "임의값 재생성"을 눌러도 연쇄 필드 값이 유지된다 —
      `--payment-screen-test`로 확인: `#14`/`#20`/`#24`가 재생성 전후 완전히 동일(텍스트 비교).
- [x] 같은 조작에서 kiosk 자체 생성 필드는 새 값으로 바뀐다 — `#32`/`#35`/`#39`/`#41`/`#42`/`#49` 전부
      재생성 후 값 변경 확인.
- [x] 연쇄 전(선행 응답 없음) 상태에서는 재생성이 전 필드를 새로 만든다(Phase 29와 동일 동작) — 스냅샷이
      빈 딕셔너리라 구조적으로 보장(코드 리뷰로 확인, 2026-09-23).
- `dotnet build` 경고 0/오류 0, 기존 회귀 4종 전부 통과 확인(2026-09-23).

---

## P30-6. 연쇄 필드 색 구분 (View)

### 구현할 것

연쇄로 채워진 필드를 **카드리딩 필드(파란 행)와 다른 색**으로 표시한다(PRD §13.3). **설명 캡션은 두지
않는다** — 색으로만 구분하는 §12.5 기조를 따른다(P29-6 4차 재확정).

### 주의

- 기존 `IsCardReadingField`(파란 행) 강조와 **시각적으로 구분**돼야 한다 — 두 색이 헷갈리면 "원캡이
  채운 것"과 "앞 전문에서 온 것"을 착각한다.
- **compact 모드까지 확인한다** — `Themes/Layout.Compact.xaml`의 `PaymentRequestCardWidth`가 별도
  값이고, 902614는 필드가 많아 스크롤바가 항상 떠서 WrapPanel 폭 계산이 어긋난 전례가 있다(P29-6에서
  `PaymentRequestCardWidth` 180→168 조정).
- 색은 기존 `Themes/*.xaml` 리소스 체계 안에서 정의한다(하드코딩 금지).
...
## 체크포인트 2 — Opus 검증 리뷰 (P30-4·P30-5·P30-6 직후)

### 검증 결과 (2026-09-23, `checkpoint-reviewer` 독립 세션, Opus)

`git diff 4660ff8..0a7918a` 전체를 직접 재검토하고 빌드·하네스 4종을 독립 재실행. 연쇄 대상 23건
전부가 실제로 kiosk 소유 필드임을 스키마와 재대조(전제 성립), `SendAsync`의 캐시 대입 순서 재확인,
902614 `#27`/`#28` 축소 난수 범위의 안전성을 직접 계산으로 검증. 절대 제약(원캡 중계 경로 3파일,
스키마 파일, `TelegramFieldChainMap.cs`/`TelegramFieldChainConverter.cs`) 위반 없음.

확정 결함 2건, 경미 지적 3건:

- **M-1** — 같은 탭을 재전송하면 연쇄가 재적용되지 않는다. `HasResponse`가 이미 `true`인 상태에서
  다시 `true`를 대입해도 `[ObservableProperty]` setter가 값 불변으로 판단해 `PropertyChanged`를
  발생시키지 않아, `PaymentScreenViewModel.OnTabPropertyChanged`가 두 번째 전송부터 아예 호출되지
  않음(PRD §13.3 "앞 전문을 재전송하면 덮어쓴다"는 확정 동작 위반). **수정 완료**(아래).
- **M-2** — `902614` 재생성 후 `#29`(총 납부 금액)가 낡은 값으로 남는다. 재생성 복원 루프가
  `#27`→`#28`→`#29` 순으로 돌면서, `#27`/`#28` 복원 과정에서 자동 재계산된 올바른 `#29` 값을
  `#29` 자신의 스냅샷(재생성 전 낡은 값)으로 다시 덮어씀. **수정 완료**(아래).
- L-1(자기참조 합산 실패 시 오진 메시지 — 실제로 바뀐 필드가 아니라 그로 인해 재계산 실패한 `#29`가
  원인인데 엉뚱한 필드가 길이 초과라고 표시됨) — **수정 완료**.
- L-2(`ApplyChainMappingsFrom`의 변환 실패 무음 스킵 — 실 VAN 업무 실패 응답 시 어느 필드가 왜 안
  채워졌는지 추적 단서가 없음) — 코드는 그대로 두고 기존 "응답 코드를 보지 않고 연쇄한다" 열린 항목에
  같이 묶어 기록만.
- L-3(`Regenerate()`의 `902614 #27`/`#28` 초기 범위 축소 로직에 필드번호 리터럴이 그대로 남음 —
  `TelegramFieldChainMap` "필드번호를 화면에 흩뿌리지 않는다" 원칙과는 어긋나지만 기능상 문제 없음) —
  주석이 이미 충분히 근거를 남기고 있어 이번엔 보류.
- 색 구분(P30-6) `#F3E8FF`(연쇄) vs `#E8F3FF`(카드리딩) 배경색 자체는 명도가 비슷하나, 서로 다른
  패널(요청 카드/응답 행)에 나타나고 연쇄 쪽엔 뚜렷한 보라 테두리(`#C4B5FD`)가 같이 붙어 실사용
  혼동 위험은 낮다고 판단 — 결함으로 기록하지 않음.

### 수정 결과 (2026-09-23, `csharp-wpf-developer` 후속 세션)

- **M-1**: `SendAsync()` 시작(네트워크 호출 전)에서 `HasResponse = false; _lastResponseTelegram = null;`을
  먼저 세팅 — 응답 도착 시 항상 `false→true` 전이를 보장. `501008` 2회 연속 전송 검증 — 902614 탭의
  `#14`/`#20`/`#24`가 1차/2차 전송마다 서로 다른 새 응답값으로 갱신됨을 텍스트 조회로 확인.
- **M-2**: `Regenerate()`의 스냅샷 구성 시 자기참조 연쇄 대상(`902614 #29`)을
  `TelegramFieldChainMap.Entries`에서 동적으로 찾아 스냅샷 대상에서 제외. 재생성 후 `#27`+`#28`을
  직접 더해 `#29`와 일치하는지 검산 확인(245137031+779728=245916759 실측 일치).
- **L-1**: `RecomputeSelfReferencingChainTargets` 호출을 `_requestTelegram.Write` 성공 처리와 분리된
  별도 try/catch로 감싸 "연쇄 재계산 실패(필드 #N 변경으로 유발됨): ..." 메시지로 원인을 구분.
- `dotnet build` 경고 0/오류 0, 기존 회귀 4종 전부 재통과(`--pos-client-test`/`--payment-flow-test`
  171/171/`--field-chain-converter-test`/`--random-value-generator-test`). `app.manifest`는 검증 중
  일시 `asInvoker`였다가 `requireAdministrator`로 원복 확인(`git diff` 클린).
- `git diff --stat` 최종 확인: `PaymentTelegramTabViewModel.cs` 1개 파일만 변경, 절대 제약 파일 전부
  미변경.

**판정: 체크포인트 2 통과** — 확정 결함 2건 전부 수정, 경미 지적 중 1건 수정·2건 기록만/보류.
...
- 실 리더기로 `501008` → `800000` → `902614` **순서대로** 왕복시키고, **앞 전문 응답값이 뒤 전문 요청에
  실제로 실려 나갔는지** 화면과 로그 양쪽에서 대조한다. 로그는 `KFTCTaxCAP{yyMMdd}.log`, 상관키는 `#9`.
- 순서를 어긴 경우(`902614` 단독 전송)도 임의값으로 정상 왕복하는지 확인한다.
- 회귀: `--pos-client-test`, `--payment-flow-test` 통과.
- 스크린샷 확보. **주의**: 앱이 관리자 권한으로 떠 있으면 UIPI 때문에 GUI 자동화가 그 창에 입력할 수
  없다 — 전체 화면 캡처는 가능하므로 사용자에게 창을 앞으로 띄워달라고 요청한다(P29-8 전례).

### 완료 조건

- [ ] `501008` 응답의 `#17`(납세의무자번호)·`#19`(징수기관명)·`#30`(본세)이 `902614` 요청 원문의
      `#14`·`#20`·`#24` 자리에 **규칙대로 변환돼** 실려 나간 것이 로그로 확인된다.
- [ ] `800000` 응답 `#24`(수수료)가 `902614 #28`에, `#17`(카드사코드)이 `#33`에 실린다.
- [ ] `902614 #29`가 `#27`+`#28`과 일치한다(로그 원문에서 계산 검산).
- [ ] 절삭 대상 필드(`#20`/`#21`/`#37`)에 깨진 한글이 없다.
- [ ] 순서 미준수(`902614` 단독) 시에도 정상 왕복한다.
- [ ] 회귀 하네스 2종 통과.
- [ ] `dotnet build` 경고 0 / 오류 0.
- [ ] `git diff --stat`에 `PaymentOrchestrator.cs`·`TransactionQueue.cs`·`PosSocketServer.cs`가 없다.
- [ ] 스크린샷이 `docs/payment_relay/images/`에 확보된다.

---

## 완료 기준 (Phase 전체)

1. P30-1~P30-7의 모든 완료 조건 통과.
2. 결제 화면에서 3전문을 순서대로 보내면 **앞 전문 응답값이 뒤 전문 요청에 자동으로 채워진다.**
3. **원캡 중계 경로(3개 파일)가 바뀌지 않았다** — `git diff`로 확인.
4. 기존 POS 경로(키오스크 → 소켓)의 동작이 바뀌지 않는다(회귀 없음).
5. 아래 "열린 항목"이 문서에 정확히 기록된다.

## 열린 항목

- **SPEC 원문 명문 근거 부재** — PRD §3.3.2의 값 출처 중 대부분은 SPEC에 문장이 없고 사용자 확정만
  근거다(`spec_open_questions.md` §4). 구현을 막지는 않으나 **다음 SPEC 개정 때 명문으로 실을지
  판단**한다 — 그때 `spec_open_questions.md`를 먼저 고친다.
- **스텁 확장(P30-1)은 개발용 장치** — 실 VAN 연동 시 `App.xaml.cs` 한 줄 교체로 함께 사라진다.
  실 VAN이 실제로 어떤 값을 돌려주는지는 서버 준비 후 실측해야 한다(§10).
- **할부 선택 UI / `#14`·`#35` 실입력** — 범위 밖(PRD §13.9). 실 VAN 연동 시점에 함께 판단한다.
- **`#53 EMV DATA` 응답 방향** — 인터넷지로도 SET하는 공유 필드라 실서버 응답 실측이 필요하다
  (ROADMAP 미확정 #6). P30-1에서 스텁이 이 자리를 덮지 않도록 주의하는 것으로 갈음한다.
- **연쇄 적용이 응답 코드(`#7`)를 보지 않는다(P30-4, 2026-09-23 발견, 체크포인트 2 L-2로 재확인)** —
  프로토콜상 응답을 받아 파싱에 성공하면(업무적으로 실패한 응답이라도) 그대로 연쇄를 적용한다. 지금
  스텁은 항상 성공(`#7`="000")만 돌려주므로 이 Phase에서는 실제로 드러나지 않는다 — 실 VAN 연동 시
  업무 실패 응답에서도 연쇄를 적용해도 되는지(빈 값/오류값이 다음 전문에 실릴 수 있음) 재검토가
  필요하다. 같은 이유로 `PaymentScreenViewModel.ApplyChainMappingsFrom`이 변환 실패(`PosProtocolException`)
  시 그 필드를 조용히 건너뛰는데(로그·화면 피드백 없음), 실 VAN 연동 시 이것도 같이 재검토한다.

## 착수 순서 요약

**P30-1(스텁 확장)** → P30-2(매핑 테이블) → P30-3(변환기) → **체크포인트 1(Opus 검증)** →
P30-4(캐시+자동채움) → P30-5(재생성 분리) → P30-6(색 구분) → **체크포인트 2(Opus 검증)** → P30-7(검증).

**P30-1을 맨 앞에** 두는 것이 이 순서의 핵심이다 — 스텁이 업무부를 채워주지 않으면 이후 모든 Task의

...
- [ ] 3전문 각 1회 왕복 성공(실기).
- [ ] `800000 #14` BIN과 `902614` 8개 필드가 실제 카드 데이터로 채워진 것이 확인된다.
- [ ] 응답·로그에 카드정보·PIN 잔존이 없다(§4.13 규칙이 그대로 지켜진다).
- [ ] 회귀 하네스 2종 통과.
- [ ] `dotnet build` 경고 0 / 오류 0.
- [ ] 스크린샷이 `docs/payment_relay/images/` 또는 스크린샷 폴더에 확보된다.

---

## 완료 기준 (Phase 전체)

1. P29-1~P29-8의 모든 완료 조건 통과.

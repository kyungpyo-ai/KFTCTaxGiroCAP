...
- [ ] `902614` **relay 응답**의 `#45`/`#46`/`#51`/`#53`이 전부 space다(raw 바이트로 확인).
- [ ] `902614` **실패 응답**(카드 채움 후 `D0x`/`E99`)의 같은 4개 필드가 전부 space다 — **위 ★ 유출
      1건이 실제로 막혔음을 증거로 남긴다**(수정 전 재현 → 수정 후 미재현).
- [ ] `#43`/`#44`/`#48`/`#50`과 그 외 모든 바이트는 **한 바이트도 바뀌지 않는다**(수정 전/후 응답
      바이트 diff가 정확히 4개 필드 구간에서만 발생).
- [ ] 응답 전체 길이 1500 불변, `#0`도 불변.
- [ ] `501008`/`800000` 응답은 **완전히 동일**하다(회귀 없음) — 특히 `501008`의 `#45`/`#46`/`#51`/`#53`
      (카드리딩과 무관한 DigitalBudget 업무 필드)이 삭제되지 않고 그대로 남아야 한다.
- [ ] **전송(POS 응답 바이트)에서는** 지운 4개 필드 값이 나가지 않는다. **로그 파일은 이 조건에서
...
- [ ] 거래 1건이 끝나면 행이 정확히 1개 존재하고, 다음 거래 후에도 여전히 1개다(값만 바뀜).
- [ ] 저장된 BLOB이 **POS로 나간 응답 본문과 바이트 단위로 동일**하다.
- [ ] 저장된 BLOB에 `#45`/`#46`/`#51`/`#53` 값이 없다(전부 space).
- [ ] DB 파일을 읽기 전용으로 만들거나 잠근 상태에서도 **거래가 정상 완료**되고 경고만 남는다.
- [ ] 저장용 복사본이 `SecureClear`로 지워진다.
- [ ] 앱을 껐다 켜도 행이 남아 있다.
...
- [ ] 앱 기동 시 `ValidateAtStartup()`을 통과한다(스키마 오류가 있으면 기동 자체가 실패해야 한다).
- [ ] 요청 본문이 정확히 70바이트, 결과 없음 응답이 정확히 80바이트다.
- [ ] `902614` 원문을 실은 응답이 1580바이트이고, 바깥 `#0`=1580 / 안쪽 원문의 `#0`=1500으로 **서로 다른
      값이 정확히 들어간다**(PRD §3.4.5 주의 문단).
- [ ] `#4` 임시값이 `501008`/`800000`/`902614`와 겹치지 않는다.
...
- [ ] 조회 요청이 카드 리딩·VAN 호출·알림창을 **전혀** 유발하지 않는다(로그로 확인).
- [ ] `#9` 일치 → 저장 원문과 **바이트 단위로 동일한** 꼬리를 실어 응답한다.
- [ ] `#9` 불일치 / 기록 없음 → `#7`이 `E07`, 개별부가 `000000` + `0000`, 본문 80바이트.
- [ ] 결과 없음 응답의 어느 바이트에도 `0x00`이 없다.
- [ ] 같은 조회를 연속 3회 보내도 매번 같은 응답이 오고 저장소가 바뀌지 않는다.
- [ ] `Services/Payment/`에 `"E07"` 리터럴이 없다(grep).
- [ ] 조회가 원거래를 추월하지 않는다 — 거래 처리 중에 조회를 밀어 넣으면 원거래 완료 후에 처리된다.
...
- [ ] 연결 강제 종료 → 재연결 → 조회로 원래 결과를 화면에 표시하는 흐름이 시뮬레이터만으로 재현된다.
- [ ] 조회 응답의 꼬리를 기존 파서에 그대로 넣어 필드가 정상 파싱된다(파서를 고치지 않는다).
...
- [ ] **① 정상 거래 후 조회** — 원문이 원래 응답과 바이트 단위로 동일.
- [ ] **② 응답 유실 복구** — 응답 직전 연결 강제 종료 후 재연결·조회로 결과 복구(P26-5 시나리오).
- [ ] **③ 다른 `#9`로 조회** → `E07`.
- [ ] **④ 기록이 없는 상태(첫 실행)에서 조회** → `E07`.
- [ ] **⑤ 앱 재시작 후 조회** → 여전히 성공(메모리 저장이었으면 실패했을 케이스).
- [ ] **⑥ 카드/PIN 잔존 전수 검사** — 응답 프레임과 DB BLOB의 raw 바이트에서 카드번호 패턴·PIN
      값이 검출되지 않는다. Phase 21 P21-2와 **같은 방식**(가짜 부품 + raw 바이트 전수 검사)을 쓴다.
- [ ] **⑦ 회귀** — 기존 `--payment-flow-test` 하네스와 `--memory-clear-test`가 전부 통과한다.
- [ ] `dotnet build` 경고 0 / 오류 0.

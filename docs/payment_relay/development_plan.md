...
- **연쇄 적용이 응답 코드(`#7`)를 보지 않는다(P30-4, 2026-09-23 발견)** — 프로토콜상 응답을 받아
  파싱에 성공하면(업무적으로 실패한 응답이라도) 그대로 연쇄를 적용한다. 지금 스텁은 항상 성공(`#7`=
  "000")만 돌려주므로 이 Phase에서는 실제로 드러나지 않는다 — 실 VAN 연동 시 업무 실패 응답에서도
  연쇄를 적용해도 되는지(빈 값/오류값이 다음 전문에 실릴 수 있음) 재검토가 필요하다.

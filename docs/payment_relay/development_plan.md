...
- **`902614` 스키마에만 있는 필드다.** `501008`/`800000`에는 아예 없으므로 기존 `#51` 처리와 같이
  "스키마에 있을 때만 지운다"로 쓴다 — 전문 종별 분기를 새로 만들지 않는다.
...
- 삭제 사실을 로그에 남긴다(값이 아니라 **어느 필드를 지웠는지**만). 기존 `TelegramLogRedactor`는
  그대로 둔다 — 로그 마스킹과 전송 삭제는 다른 축이다.
...
- [ ] `501008`/`800000` 응답은 **완전히 동일**하다(회귀 없음).
- [ ] 지운 값이 어떤 로그에도 남지 않는다.

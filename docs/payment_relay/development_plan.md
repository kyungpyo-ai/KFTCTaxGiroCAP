...
5. **계층 규칙**(`ROADMAP.md` "계층 구조"): `Services → Protocol → Interop` 단방향. `Services`는 바이트
   오프셋을 직접 다루지 않는다.
...
충족되는지는 **실제로 로드해봐야만** 알 수 있으므로, 리스크를 Phase 16까지 미루지 않고 여기서 드러낸다.
...
- 여기서는 **로드만** 한다. 함수 호출은 Phase 8(Reader)/Phase 16(VAN)에서.
...
- [ ] `KFTC_GIRO.dll` 로드 결과를 확인하고 기록한다 — **실패한다면 그 사유(특히 MFC42 계열 의존성)를 이
      Task 아래에 적고 사용자에게 보고한다.** Phase 16 전에 해결돼야 할 환경 문제다
...
# Phase 8 — Reader DLL P/Invoke 바인딩 + 파일럿(`0x60`)
...
## P8-1. `Interop/ReaderSerialNative.cs` 포팅
...
- `data`는 `byte[]`가 아니라 **`IntPtr`로 받는다**(참조 구현과 동일). 마샬러의 자동 복사에 기대지 않고
  `Marshal.Copy` 호출 지점을 코드에 드러내기 위함 — 데이터 수명 규칙(P8-2)을 코드로 강제하는 장치다.
...
- **검증 트리거**: 리더기 설정 화면의 "초기화" 버튼을 **임시로** 이 경로에 연결한다(기존 3초 타이머 스텁
  대신). 정식 배선·문구·실패 처리는 Phase 11에서 하므로 여기서는 최소한으로만 두고 `TODO` 주석을 남긴다.
  별도의 일회용 진단 UI를 만들지 않는 이유는, 어차피 Phase 11에서 이 버튼을 쓰게 되므로 그쪽으로 자연스럽게
  이어지기 때문이다.

**완료 조건**
- [ ] **실장비에서 `0x60` → `0x70` 왕복 성공**을 확인하고 로그를 남긴다
- [ ] 응답 `data` 첫 2byte를 업무 응답코드로 읽어 성공/실패를 판정한다
- [ ] 실장비가 없으면 이 Task를 **미완료로 두고** 확인하지 못한 범위를 여기에 적는다(P8-4는 하드웨어 없이도
      가능하므로 먼저 진행 가능)

## P8-4. 오류 경로 검증 (하드웨어 없이 가능)
...
- [ ] `AccessViolationException`이나 `BadImageFormatException`이 발생하지 않음(발생하면 P8-1 시그니처
      대조로 되돌아간다)

---

# Phase 9 — Reader 서비스 계층

**이 Phase가 끝나면**: 결제 Flow와 리더기 설정 화면이 공용으로 쓸 리더기 제어 계층이 완성된다. 이 Phase의
**단일 유효 응답 게이트**(P9-4)가 이 프로젝트 전체에서 가장 중요한 구조물이다.

## P9-1. `Protocol/Reader/` 응답 파서
...
- `0x3B`(카드 리딩 응답): 응답코드(`00`/`07`/`12`/그 외)와, VAN 요청에 필요한 카드 데이터. VAN 전문이 아직
  미확정(PRD §10)이므로 **파싱 결과를 구조화해 보관만** 하고 VAN 매핑은 Phase 16에서 한다.
...
## P9-2. `Protocol/Reader/` 요청 빌더 (`0x2B`)
...
## P9-3. `Services/Reader/ReaderService` + 재시도 래퍼
...
3. 취소/Timeout과의 경합 → 하나만 확정 (PRD §8.3, Phase 15에서 이 게이트를 **확장**)
...
- 먼저 최종 응답한 리더기를 채택(P9-4 게이트) → 아직 대기 중인 나머지에 **`0x60` 전송으로 무효화**.
...
**이 Phase가 끝나면**: 무결성 체크 결과를 저장하고, Phase 11(리스트 표시)·Phase 14(결제 선행 판정)가 쓸
조회 API가 준비된다.

## P10-1. SQLite 패키지 선정 및 x86 로드 검증

SQLite는 **네이티브 라이브러리를 동반**하므로 Phase 7의 x86 고정과 충돌하지 않는지 먼저 확인해야 한다.
...
- DB 파일 위치: `%LOCALAPPDATA%\KFTCTaxGiroCAP\`(P7-3 로그와 동일한 이유 — 설치 폴더 쓰기 권한 문제 회피).
- 최초 실행 시 파일·테이블 자동 생성.
- POS 식별번호는 `KFTCTAXGIROCAP01` **하드코딩 상수**(PRD §2.1)를 그대로 기록한다.
- **조회 성능 관점**: "금일·특정 COM Port·성공" 조회(P10-3)가 결제마다 실행되므로 그 조건에 인덱스를 둔다.
...
Phase 8의 실장비 검증 결과와 P10-1의 SQLite 선택 결과를 반영해 **Phase 11~17 실행계획서를 이어서 작성**한다.
이 시점에 다음이 확정되어 있어야 한다.

- `KFTC_GIRO.dll` 로드 가능 여부(P7-4) — 실패했다면 Phase 16 전에 환경 문제 해결 필요
- 실장비 리더기 대수(1대/2대) — 이중화 검증 범위 결정
- `0x71` 파싱 결과(리더기 인증 식별번호/모듈 ID 필드 구조) — Phase 11 화면 출력에 필요

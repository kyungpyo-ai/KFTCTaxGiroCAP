...
- [ ] `501008` 전송 성공 직후 `902614` 탭의 `#14`/`#20`/`#24` 등이 응답값으로 바뀐다(화면 확인).
- [ ] `800000` 전송 성공 직후 `902614` 탭의 `#28`/`#33`이 바뀌고, `#29`가 `#27`+`#28`로 재계산된다.
- [ ] `501008`을 보내지 않고 `902614`를 먼저 보내도 임의값으로 정상 왕복한다(PRD §13.4).
- [ ] 창을 닫고 다시 열면 캐시가 비어 있다(처음부터 전송해야 한다).
- [ ] `git diff`에 `PaymentOrchestrator.cs`·`TransactionQueue.cs`·`PosSocketServer.cs`가 없다.

...
- [ ] 매핑 테이블이 한 파일에 모여 있고, `ViewModels/`·`Views/` 어디에도 연쇄용 필드번호 리터럴이
      없다(grep으로 점검).
- [ ] 테이블 항목 수와 PRD §3.3.2 표의 행 수가 일치한다(누락 없음).
- [ ] 각 항목에 근거 주석(SPEC 명문인지 사용자 확정인지)이 붙어 있다 —
      `spec_open_questions.md` §4의 "SPEC 명문" 열과 대응.

...
- [ ] 3전문 전부에 대해 생성 → `PosField.Pad()` → `ToBody()`가 예외 없이 통과하고, 본문 길이가
      스키마 총 길이(706/500/1500)와 정확히 일치한다.
- [ ] 생성된 본문에 `0x00`이 없다(§4.10 금지 규칙과 같은 기준).
- [ ] `N` 필드는 숫자만, `AHN` 필드는 CP949 인코딩 후 길이가 선언 길이를 넘지 않는다.
- [ ] 원캡 담당 필드(`800000 #14`, `902614` 8개)가 전부 공백이다.
- [ ] 한글 바이트 길이 경계 테스트가 있고 통과한다.

...
- [ ] 절삭 경계 케이스가 전부 통과한다:
      ...
      한도를 1바이트만 넘는 경우 / 빈 값 / 한도보다 짧은 값(절삭 없음).
- [ ] 절삭 결과를 CP949로 인코딩했을 때 **항상 한도 이하**이고, 디코딩하면 깨진 글자가 없다.
- [ ] 합산 결과가 `PosField.Pad()`를 통과하고, 자리수 초과 시 예외가 난다(조용히 잘리지 않는다).
- [ ] 진단 하네스 인자로 이 검증을 재실행할 수 있다(기존 `--random-value-generator-test` 방식을 따른다).

...
- [x] 원캡 담당 필드(`800000 #14`, `902614` 8개)가 전부 공백이다.
...
- [ ] 창이 열리고 3전문 전부에서 요청 필드 표가 임의값으로 채워져 보인다.
- [ ] 카드리딩 필드가 읽기전용·공백으로 표시된다.
- [ ] `전송` 후 응답 필드 표와 원문 패널이 채워지고, `#0` 길이가 본문 길이와 맞는다.
- [ ] 응답 패널에 "VAN 스텁 = 요청 에코" 경고가 보이고, 스텁이 덮어쓴 `#3`/`#6`/`#7`/`#8`이 구분된다.
- [ ] `902614` 전송 시 결제 알림창이 이 창 위에 뜨고, 취소·ESC가 그대로 동작한다.
- [ ] 코드비하인드에 비즈니스 로직이 없다(창 핸들 관련 코드만).
- [ ] 로그에 카드정보·PIN 원문이 남지 않는다(`902614` 전송 후 로그 파일 확인).

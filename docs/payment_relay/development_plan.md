...
- [ ] 홈 "결제" 카드 설명이 "결제 화면을 실행하고\n결제를 진행합니다"로 표시되고 `PRD_WPF.md`도 갱신됐다.
- [ ] 홈 "결제" 카드 클릭 → 결제 화면이 열린다(MessageBox가 더는 뜨지 않는다).
- [ ] 두 번 클릭해도 창이 하나만 존재한다.
- [ ] 결제 화면이 열린 상태에서 소켓으로 들어온 전문이 `E03`으로 거절되지 않는다.
- [ ] 리더기/가맹점 설정 화면의 기존 `E03` 동작은 그대로다(회귀 없음).

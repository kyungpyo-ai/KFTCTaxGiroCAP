...
- [ ] `501008` 1건 전송 → 응답 프레임 수신이 성공한다(리더기 불필요).
- [ ] `902614` 전송 시 카드리딩·PIN을 기다리는 동안 클라이언트가 타임아웃으로 먼저 끊지 않는다.
- [ ] 서버가 응답 없이 연결을 닫는 경우에도 클라이언트가 예외로 죽지 않고 오류를 돌려준다.
- [ ] UI 스레드 블로킹이 없다(전송 중 창이 응답한다).

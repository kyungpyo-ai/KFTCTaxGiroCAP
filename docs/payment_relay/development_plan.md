...
#11/#12/#14/#16)만 남은 것을 스크린샷으로 확인(`payment_screen_ownership_fix.png`). 응답 표(전송 후
채워지는 목록)에 `#3`/`#6`/`#8`이 다시 포함되는지는 UIPI로 "전송" 클릭이 막혀 **아직 실측하지 못했다**
— `_responseFieldNumbers` 계산 로직은 코드로 확인했다(`HasNonKioskOwner(field)`가 `#3`/`#6`/`#8`에 대해
true를 반환함을 `Owners` 값으로 직접 추적).
...
- [x] **(2026-09-18 재확정) 카드리딩 필드는 요청 표에 아예 나타나지 않는다**(회색 읽기전용이 아니라
      ...
      표시된다")은 이 확정으로 대체됨.
- [ ] `전송` 후 응답 필드 표가 채워진다 — **UIPI로 자동 클릭 확인 불가, 실측 필요.**
- [x] 응답 패널에 "VAN 스텁 = 요청 에코" 경고가 보이고, 스텁이 덮어쓴 `#3`/`#6`/`#7`/`#8`이 구분된다 —
      **(2026-09-18 3라운드) `#3`/`#6`/`#8`은 kiosk와 다른 주체가 같이 체크된 필드라 요청·응답 양쪽에
      다 나타난다**(`HasNonKioskOwner` 코드로 확인, 화면 실측은 미완). 경고 배너는 조건 없이 항상
      표시된다(스크린샷으로 확인).

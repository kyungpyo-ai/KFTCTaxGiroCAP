...
**완료 조건**
- [ ] 취소와 Timeout이 **같은 메서드**를 호출해 정리한다(경로 2개가 아님)
- [ ] 리더기 2대가 대기 중일 때 취소/Timeout 각각에서 **2대 모두** `0x60`을 받는다(하네스 카운트로 확인)
- [ ] `0x60` 발사가 UI 스레드에서 일어나지 않는다(코드 리뷰 + 취소 직후 알림창이 멈추지 않음을 실기 확인)

...
각 Phase의 Task 단위 작업 지시/완료 조건 체크리스트는 **별도 실행계획서**(`docs/payment_relay/development_plan.md`,
...
- [ ] 빌드 산출물 옆에 두 DLL이 복사되도록 csproj 배선 — 빌드에 쓰이는 사본의 위치 결정 필요:
      `vendor/ReaderSerial/`은 이미 빌드 자산 성격이나, `KFTC_GIRO.dll`은 현재 `docs/payment_relay/dll/`
      (문서 폴더)에 있다. **`vendor/KftcGiro/`로 옮겨 `vendor/` 아래로 통일**할지 결정한다(권장 — `docs/`는
      문서, `vendor/`는 외부 바이너리라는 기존 구분 유지)
...
- [ ] `Reader_OpenPort`/`Reader_ClosePort`/`Reader_SendCommand(0x60)` 왕복 — 응답 `0x70`의 업무 응답코드
      (`data` 첫 2byte, ASCII)를 읽어 성공/실패 판정
...
- [ ] `Services/Reader/ReaderService.cs` — 포트 열기/닫기, 명령 송신, 콜백 정규화(이벤트/결과 객체로 변환)
- [ ] 재연결 래퍼(`SendCommandSafe` 패턴, `docs/reader_dll/DLL연동가이드.md` §5): 포트 계열 오류에서만
      Close→Open→1회 재시도, `READER_ERR_BUSY`는 복구 대상 아님, 재오픈 시 **새 `readerId`로 덮어쓰기**
- [ ] **단일 유효 응답 게이트** (PRD §8.2): 한 요청에 대해 콜백이 중복 도착해도 최초 1건만 처리한다.
      Phase 15의 취소/Timeout 경합도 이 게이트를 재사용하므로 여기서 구조를 잡는다
...
`Services/`가 바이트 오프셋을 직접 다루지 않는지(계층 규칙) 점검한다.
...
여기서 함께 해소한다 — 이 항목들은 `docs/payment_relay/PRD.md`에 아직 없으므로 착수 전 PRD 갱신이 필요하다
(아래 "확인 필요" 참고).
...
- [ ] 〔1차 보류〕실제 COM 포트 열거 (`PRD_WPF.md` 4.13) — `SerialPort.GetPortNames()`, "미사용" 첫 항목,
      저장된 포트가 목록에 없으면 `"<port>(사용불가)"`로 유지
- [ ] 〔1차 보류〕포트 열기 토글 (`PRD_WPF.md` 4.8) — 실제 `Reader_OpenPort`/`Reader_ClosePort`,
      리더기2 인터락, `PORT_ALWAYSOPEN` 반전 인코딩 저장, 확인창 정식 한글 문구 확정
- [ ] 〔1차 보류〕AOP 제약 (`PRD_WPF.md` 4.11) / TRANSINFO_AOP 확인 시 검증 (`PRD_WPF.md` 4.12)
- [ ] 로딩 스피너·동시 1작업 제한 등 기존 UI 규약(`PRD_WPF.md` 4.7) 유지 — 3초 타이머만 실통신으로 교체
...
- [ ] `Views/PaymentNoticeWindow` 신설 — IC(`BG_IMG_IC`)/FALLBACK(`BG_IMG_MS`)/통신중(`BG_IMG_PROCESSING`)
      3개 상태 전환 (PRD §5.2)
- [ ] 이미지 자산 배치 결정: 원본은 `docs/payment_relay/images/`에 보관 중이며, 앱이 쓰는 사본을
...
- [ ] 무결성 선행 판정 (PRD §4.2): 금일·동일 COM Port 성공 이력 없으면 무결성 체크 수행 후 진행
- [ ] IC 정상 (PRD §4.3): 알림창 IC → `0x2B`(거래구분 `ARQo`) → `0x3B` 응답 `00` → VAN 요청 데이터 생성
- [ ] FALLBACK (PRD §4.4): `0x3B` 응답 `07` → 알림창 MS로 전환 → `0x2B` 재요청(거래구분 `F`)
- [ ] 응답 `12` (PRD §4.5): `0x2B` 재요청(거래구분 `ARQo` 유지)
...
- [ ] 결제 대상 포트 결정 규칙 확정 — 아래 "확인 필요" 참고

**완료 기준**: 정상/FALLBACK/`12` 재시도/기타 실패/DLL 오류 5개 시나리오가 각각 올바른 분기로 끝나고, POS에
구분 가능한 결과가 반환된다. 연속 2건 거래 시 앞 거래 데이터가 뒤 거래에 섞이지 않는다.
...
- [ ] 사용자 취소 (PRD §4.8): 취소 버튼/ESC → 리더기 초기화(`0x60`) → POS에 취소 응답
- [ ] 카드 입력 Timeout 120초 (PRD §4.9): 자체 타이머 → 리더기 초기화 → POS에 Timeout 응답
- [ ] 단일 결과 확정 게이트 — Phase 9의 게이트를 확장해 다음 경합에서 1건만 처리:
      카드리딩 완료+취소 / 카드리딩 완료+Timeout / 취소+Timeout / 콜백 중복
- [ ] 거래 종료 시 타이머·콜백·ESC 후킹 등 리소스 해제 (PRD §9)

**완료 기준**: 경합 4종을 인위적으로 재현해 각각 결과가 1건만 확정되고 나머지는 무시되는 것을 확인한다.
반복 실행 시 타이머/후킹이 누적되지 않는다.
...
- [ ] Mode(`R`/`IT`/`OT`) 선택 경로 + `KFTC_GIROPOS.ini` 설정 확인 (PRD §2.3, §10 — INI 파일 미확보 상태)
...
## 착수 전 확인 필요 (PRD 갱신 대상)

로드맵을 작성하며 발견한, **`docs/payment_relay/PRD.md`에 아직 답이 없는 항목들**이다. 해당 Phase 착수 전에
사용자 확인 후 PRD를 먼저 갱신한다(ROADMAP 규칙 3: 코드보다 PRD를 먼저 고친다).

1. **결제 시 사용할 리더기/포트 결정 규칙** (Phase 14) — PRD §4.2는 "현재 사용 중인 COM Port"라고만 한다.
   리더기 설정 화면은 리더기1/2 두 개를 다루는데, 결제는 어느 쪽을 쓰는가? 리더기1 고정인가, 별도 규칙이 있는가?
2. **"포트 열기"(`PORT_ALWAYSOPEN`)와 결제 Flow의 포트 생명주기 관계** (Phase 11·14) — `PRD_WPF.md` 4.10의
   설명은 "ON=항시 연결, OFF=거래시에만 연결"이다. 즉 이 설정이 결제 시 포트를 언제 열고 닫을지를 직접 규정하는데,
   `payment_relay/PRD.md`에는 이 언급이 없다.
3. **1차 범위 보류 항목 4종의 이번 범위 포함 여부** (Phase 11) — 실제 COM 포트 열거(4.13), 포트 열기 토글(4.8),
   AOP 제약(4.11), TRANSINFO_AOP 검증(4.12). 앞의 둘은 DLL 연동이 전제라 이번 범위가 자연스럽지만, 뒤의 둘은
   레지스트리 로직이라 DLL과 무관하게 보류돼 있었다 — 함께 처리할지 확인 필요.
4. **알림창 크기 조절 트리거** (Phase 12) — 자산만 확보됨(PRD §10에 미확정으로 기록). 이번 범위 밖으로 두고
   두 크기 자산만 준비하는 것으로 계획했다.
5. **POS 식별번호의 설정 가능 여부** (Phase 10) — PRD §2.1은 기본값 `KFTCTAXGIROCAP01`만 제시한다. 사용자가
   변경할 수 있어야 하는지, 고정값인지 확인 필요.
...
- `docs/payment_relay/development_plan.md` — 2차 범위 Task 단위 실행계획서(Phase 착수 시 작성)

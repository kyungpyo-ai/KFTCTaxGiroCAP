...
| 3 | 홈 화면 — 인터랙션 & 트레이 | ✅ 완료 |
| 4 | 리더기 설정 — 정적 레이아웃 | ✅ 완료 |
| 5 | 리더기 설정 — 비즈니스 로직(스텁) + 포트열기 토글/레지스트리 (AOP 제약·TRANSINFO_AOP 검증은 보류) | ⬜ 대기 |
...
- [x] "포트 설정" 섹션 카드: 리더기1/2 카드 2개 정적 배치(콤보, 버튼 5개, info 버튼) — PRD 4.4. **포트 열기 토글은 이번 Phase에서 만들지 않음**(Phase 5에서 비즈니스 로직과 함께 구현 — 사용자 명시적 지시).
...
## Phase 5 — 리더기 설정: 비즈니스 로직(스텁) + 포트열기 토글 & 레지스트리

> **2026-08-14 범위 조정(사용자 지시)**: 기존에 별도 Phase였던 "비즈니스 로직(스텁)"과 "포트열기 토글 & 레지스트리 연동"을 하나의 Phase로 합쳤다. 대신 아래 두 항목은 이번 Phase에서 **명시적으로 제외**하고 이후 별도 단계로 미룬다(AOP 관련 로직은 레지스트리 `INTERLOCK` 값 읽기·카드 뱃지 표시 등 아직 이 저장소에 없는 선행 작업이 얽혀 있어 별도로 다루는 게 안전하다는 판단):
> 1. AOP 제약 로직(PRD 4.11) — `INTERLOCK` 값에 따른 컨트롤 활성/비활성 전이. 미구현.
> 2. 확인(OK) 검증 중 TRANSINFO_AOP 모드 포트 미지정 시 저장 차단. 미구현.
>
> 리더기2 강제 비활성화(단일 포트 인터락, 아래 체크리스트)도 "TRANSINFO_AOP 예외"만 이번 범위에서 제외 — 예외 없는 기본 인터락 자체는 이번 Phase에서 구현한다.

**목표**: PRD 4.7~4.10, 4.13의 로직을 스텁 수준으로 이식(원본도 실통신 미구현) + PRD 4.8 포트 열기 토글의 확인창 → 백그라운드 처리 → 타임아웃 → 레지스트리 저장 흐름.
...
- [ ] 포트 열기 토글(리더기1 전용, Phase 4에서 자리만 비워두고 보류했던 항목) 신규 구현: 클릭 → 확인창(문구는 PRD 미확정 사항 #2 확정 후 반영) → 비동기 처리(2.5초 시뮬레이션) + 10초 타임아웃 → 되돌림/재적용
- [ ] 성공 시 리더기2 강제 비활성화(단일 포트 인터락 기본 동작만 — TRANSINFO_AOP 예외 처리는 위 범위 조정에 따라 제외)
- [ ] 레지스트리 저장: `COMPORT1_FIELD`/`COMPORT2_FIELD`, `MULTIPAD1_FIELD`/`MULTIPAD2_FIELD`(반전 인코딩), `PORT_ALWAYSOPEN`(반전 인코딩) — PRD 5장 참고, 레지스트리 공유 여부는 미확정 사항 #1 확인 후 최종 확정
- [ ] 정보 팝오버(포트열기/멀티패드) — PRD 4.10 문구 그대로
- [ ] 스냅샷/dirty-check(PRD 4.13): 콤보1/2 + 멀티패드1/2(+포트열기) 추적, 취소 시 확인창

**완료 기준**: 각 액션 버튼/조회 버튼 클릭 시 로딩→완료 흐름 확인. 포트 열기 토글 ON/OFF 흐름이 원본과 동일하게 동작하고 레지스트리 값 확인. dirty-check(콤보/토글 변경 후 취소 시 확인창) 동작 확인. (AOP 시나리오 검증, TRANSINFO_AOP 저장 차단은 위 범위 조정에 따라 이번 완료 기준에서 제외 — 별도 단계에서 다룸)
